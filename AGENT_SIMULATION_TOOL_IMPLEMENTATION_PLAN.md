...
Agent 需要新增"主动执行电路仿真"的能力，且定位是与其他 tool 完全解耦的通用能力——必须能对**项目工作区内任意一个已有电路文件**发起仿真，而不是只能仿真用户当前在编辑器里打开的那一个。在此之上，后续还会陆续新增 `read_metrics` / `read_signals` / `read_output_log` / `read_op_result` 等一系列工件读取 tool；其中 `read_signals` 是 agent 读取仿真数值信号的**唯一入口**，统一覆盖 `raw_data` 与可选 `charts` 视图，文档中不再规划独立的波形 / 图表 read tool。这些 tool 之间必须互相独立，可以在任意时刻任意顺序被调用，不依赖任何"当前仿真"session 的隐式状态。
...
- **显示态图表 / 波形工件与 agent 权威数据面边界不清**：`SimulationExportCoordinator` 里的 `_chart_viewer.export_bundle(...)` / `_waveform_widget.export_bundle(...)` 依赖实时 Qt 组件；它们表达的是 UI 当前显示 / 导出的视图，不适合作为 agent 的权威信号源。Agent 在后台 origin=AGENT_TOOL 的 job 完成后真正必须无头落盘的是 `raw_data` / `metrics` / `output_log` / `analysis_info` / `op_result`（若适用）；`charts/` 与 `waveforms/` 只能作为**可选** display artifact，在存在时被 `read_signals` 或附件链路附加利用，而不是单独 read tool 的前置条件

因此本计划在 job 架构之前先开第 0 步把"单树磁盘布局 + 无头权威 artifact 边界"这两件事一次性钉死，再做 job 架构改造；否则 Step 14 起的 agent tool 链路即使设计再优雅也没有合格数据源。
...
- **`export_root`**：`simulation_results/<stem>/<ts>/` 目录本身，承载所有该次仿真的 artifact。对 agent 而言，`result.json`、metrics、analysis_info、raw_data、output_log 与 `op_result`（若适用）构成**无头权威工件**；`charts/`、`waveforms/` 及 conversation snapshot 只在 UI / 附件链路需要时附加存在。目录内文件布局视为稳定 schema，`result.json` 与各工件子目录共根存放。

- **`artifact bundle`**：`export_root` 目录及其子目录里的全部文件的合称。第 0 步新建的 **无头工件持久化服务**（`SimulationArtifactPersistence`）在任何 job 完成时写出权威工件；display artifact 可在 UI 导出或附件链路中附加出现。**Agent 的 `read_*` tool 所依赖的，是这份 bundle 里的权威工件；`read_signals` 只在 chart CSV 实际存在时把它们当成可选特化视图使用**。
...
- **新服务 `SimulationArtifactPersistence`**（位于 `domain/simulation/data/` 下）：职责是"给定 `export_root` + `SimulationResult` + metrics 上下文，**无头**地写齐 agent 与面板共同依赖的权威工件"——metrics、output_log、analysis_info、raw_data、op_result（若适用）、`export_manifest.json`。它只写**不依赖 UI 显示状态**的数据产物；`charts/` 与 `waveforms/` 保留为 display artifact，不再被塞进 headless 完整性定义
...
- **UI `ExportPanel` 角色彻底收窄**：不再承担"自动导出到项目目录"职责，只保留"手动导出到用户选择的外部目录"职能；`ExportPanel.auto_export_to_project` / `SimulationTab._auto_export_current_result` 整体删除。UI 的 chart 切换、waveform 渲染依然使用 Qt widget；若这些视图需要额外落盘，由 display/export 或附件链路显式触发，但这属于**展示**逻辑，不再混入 headless 持久化定义
...
单树合并只改变"`result.json` 放在哪里"，并把"无头权威工件"与"可选 display artifact"的边界写清。现状结构视为长期合约，`SimulationArtifactPersistence` 负责写出权威工件；UI 导出 / 附件链路可以在此基础上补充 display artifact。权威清单（每次成功仿真都必须存在）：
...
可选 display artifact（仅在 UI 导出 / 附件链路触发时出现，不作为 agent read 前提）：

- `simulation_results/<stem>/<ts>/charts/<NN>_<chart_type>.png` + `.csv` + `.json`（`NN` 两位补零；若存在，则 `read_signals(source='chart', chart_index=...)` 可按 `charts.json` manifest 精确寻址）
- `simulation_results/<stem>/<ts>/charts/charts.json`（chart manifest）
- `simulation_results/<stem>/<ts>/charts/current_chart.png`（**保留**；对话附件 `attach_chart_image` 依赖此固定名）
- `simulation_results/<stem>/<ts>/waveforms/waveform.png` + `waveform.csv` + `waveform.json`
- `simulation_results/<stem>/<ts>/waveforms/current_waveform.png`（**保留**；对话附件 `attach_waveform_image` 依赖此固定名）

**自证策略**：一次基线仿真跑两遍——Step 0 改造前与改造后——先对无头权威工件集合做 `find . -type f | sort` 对比（忽略 `<ts>` 本身的时间戳差、忽略 `result.json` 位置变化），确认 mandatory set 完整且路径稳定；再在触发 UI 导出 / 附件链路的情况下对 display artifact 的命名与固定路径做单独断言。此断言写进第 22 步回归测试。
...
目标：未来的 `read_metrics` / `read_signals` / `read_output_log` / `read_op_result` 等 tool 能完全无状态、互相独立，不依赖任何共享 session 就能工作。
...
- 新增一个 `base_parameters()` 构造器供四个 tool 复用参数 schema，避免四份 `result_path / circuit_file` 字段拷贝；每个 tool 只在上面追加自己专属参数（如 `read_output_log` 的 `section`、`read_signals` 的 `source / chart_index`）
- 自证步骤：加载到的 `SimulationResult` 元数据与 LLM 传入的 `circuit_file` 做核对，若不一致以 result.json 为准、在返回 content 里显式告知差异，避免 LLM 以为"读错了"而反复重试
- 缺失 artifact 的兜底：目标文件不存在时 is_error，content 建议一个具体的替代 tool（例如 `.op` 分析不该走 `read_signals` 的 chart / waveform 语义时，提示 `read_op_result` 或 `read_metrics`），让错误路径也给 LLM 清晰的下一步
...
- **禁止"顺手读相邻 artifact"**：`read_metrics` 不许顺带返回 signal 摘要、`read_signals` 不许顺带回 metrics / output_log；职责单一
...
- 修改：`domain/llm/agent/agent_prompt_builder.py`（guideline：改完电路跑完 `run_simulation` → 优先用 `read_metrics` 看是否达标；拿到未达标项再决定是否深入 `read_signals` / `read_output_log`）
...
### 第 18 步：`read_signals` tool

目标：让 LLM 通过**一个统一入口**读取仿真的数值信号，而不是在旧的波形入口 / 图表入口 / UI overlay 之间来回猜。`read_signals` 默认读仿真器的权威全量转储 `raw_data/raw_data.csv`，在存在 `charts/` manifest 时还能窄读某一张 chart 对应的特化 CSV；返回的始终是数值统计与锚点样本，而不是 PNG base64。

#### 实施点

- 入口走第 16 步公共基座
- `source` 参数统一为 `auto | raw | chart`：
  - `auto`（默认）始终解析到 `raw_data/raw_data.csv`，这是 agent 的**权威默认信号源**，与 UI 当前勾选状态无关
  - `raw` 显式要求读全量转储
  - `chart` 需要 `chart_index`，从 `charts/charts.json` manifest 里定位对应 `<index>_<type>.csv`；这是可选特化视图，不是独立 tool
- `waveforms/waveform.csv` 明确排除在 agent 决策面之外：它是 UI overlay 的列子集与显示产物，不暴露给 `read_signals`；agent 想看某几条信号，用 `source='raw' + signal_filter=[...]` 达到同等效果
- 返回组织成 markdown：自证 header → source 摘要（当前读的是 raw 还是哪张 chart）→ series statistics 表（`name | samples | min | max | mean | initial | final | zero_crossings | peak_to_peak`）→ anchor samples 小表 → `sources_available` 概览，方便下一轮精准窄读
- `anchor_count` 与 `anchor_scale` 作为本 tool 专属参数；`anchor_scale='auto'` 时，raw 会按 `analysis_type` 选择线性 / 对数，chart 则尊重 sidecar `log_x` hint
- `.ac` 分析的 bandwidth / gain margin / phase margin 等派生量**只引用 `metrics.json`**，不从采样点重算；若当前 source 不是 Bode 语义，则不强行拼 FOM 段
- chart / waveform PNG 只作为路径字符串出现在 content 或 `details` 中，供用户查看；LLM 分析以数值表为准，绝不返回图像 bytes
- 当 bundle 里没有任何可读 signal CSV 时 → is_error；当 `source='chart'` 但没有 manifest 或 index 不存在时 → is_error 且列出 `sources_available`

#### 涉及文件

- 新建：`domain/llm/agent/tools/read_signals.py`
- 修改：`domain/llm/agent/tool_factory.py`（注册，删除任何旧的独立波形 / 图表 read tool 注册规划）
- 修改：`domain/llm/agent/agent_prompt_builder.py`（guideline：agent 读取信号统一使用 `read_signals`；首次可省略 `source`，下一轮按 `sources_available` 精读）
- 修改：`domain/llm/agent/tools/simulation_series_stats.py`（若统计 / 锚点抽样 helper 尚未抽出，收束到该共用模块）

#### 清理清单

- **字面量级删除**：文档与工具注册层面所有把 agent 信号读取写成多个独立入口的条目；`read_signals` 成为唯一权威入口
- **禁止暴露 UI overlay**：`read_signals.py` 内禁止 import / 调用 `waveforms_paths()`；grep 自证零命中
- **禁止 PNG base64 回传**：tool 文件内禁止 `base64` / `Path(...).read_bytes()` 用于 `.png`；grep 自证零命中
- **series statistics 唯一出处**：统计量与锚点采样 util 收束到共用 helper；禁止再有旧的独立波形 / 图表 tool 各自一份实现
- **禁止全量读入**：超过 10 万行 CSV 必须使用流式统计 + 锚点抽样；grep 自证 tool 内没有 `read_text()` / `readlines()` 把 CSV 全读进内存的调用
- **禁止在 `.ac` 分支里重算 bandwidth / gain margin / phase margin**：只引用 `metrics.json`；grep 自证 tool 内无这些派生量的本地计算表达式

#### 输出

- Agent 面对仿真信号只有一个心智模型：先 `read_signals(source='auto')` 读权威 raw，再按 `sources_available` 定向窄读 chart source
- 旧的独立波形 / 图表 signal tool 设计整体退出文档与实现规划
...
目标：让 LLM 能单独寻址一次 `.op` 工作点分析的节点电压、支路电流与设备偏置——这是模拟电路调优最高频的问题，但它的数据形态与 signal 统计 / 日志均不同，不能被 `read_metrics` / `read_signals` 代替，爱被第 18、19 步的错误分支引用——不单独定义即成死引用。
...
- **禁止对 AC / TRAN / DC 分析调用此 tool 时无错误返回**：非 `.op` 的 `result` 一律 is_error、引导 `read_metrics` / `read_signals`
...
### 第 20 步：`read_signals` 的可选 chart source + 主动改正的 chart 导出不良设计

#### 主动识别到的不良设计

`presentation/panels/simulation/analysis_chart_viewer.py` 的 `export_bundle()` 里 `chart_index = 1` 是硬编码的，只导出 UI 当前选中的那一张 chart，但 `export_root/charts/charts.json` 的 manifest 结构明显是为多图预留的。这是典型的"UI 视选语义漏到导出层"：如果 `read_signals(source='chart')` 依赖这份 manifest，它就只能看到用户最后一次选过的那张图，其余 chart spec 对 agent 完全不可见。

同时 `AnalysisChartViewer` 把"给哪张 chart 构造 payload"这件事与"把它画到 UI"强耦合——导致 chart 导出既不是权威的 display artifact 枚举，也无法作为 `read_signals` 的稳定可选 source。

#### 改正方向（本步必须顺手做）

- **明确边界**：agent 没有独立 chart read tool；chart CSV/JSON 只是 `read_signals(source='chart', chart_index=...)` 的**可选信号源**。`charts/` 缺失不是 agent 失败，只表示当前 bundle 没有额外 chart source
- 在 `SimulationArtifactExporter` 或共享 util 侧新增权威入口：按 `SimulationResult.analysis_type` 枚举该分析理论上可产出的 chart spec，并为每个 spec 构造统一 payload / 文件路径；viewer 不再自己决定导出集
- `AnalysisChartViewer.export_bundle` 退位为 thin 适配层：调用共享枚举 → 循环对每个 spec 落 `<index>_<type>.{png,csv,json}` → 最终写 `charts.json` 全量条目；保证 `read_signals` 看见的是"这次分析所有已导出的 chart source"，不是"UI 恰好停留的一张"
- `read_signals` 首次默认仍读 `raw_data/raw_data.csv`；若 `charts.json` 存在，则在 `sources_available` 中列出 chart index / type / title，下一轮由 agent 精确指定 `source='chart'`
- `waveforms/` 保持 UI overlay / snapshot 语义，不纳入 agent source 枚举；不要再试图通过恢复旧的独立波形 tool 把它拉回 agent 决策面

#### 涉及文件

- 修改：`domain/llm/agent/tools/read_signals.py`（确保 chart source 的发现 / 错误提示 / `sources_available` 文案与本步一致）
- 修改：`domain/simulation/data/simulation_artifact_exporter.py`（chart spec 枚举与 canonical path helper 的权威入口）
- 修改：`presentation/panels/simulation/analysis_chart_viewer.py`（`export_bundle` 改 thin 适配层；删除 `chart_index = 1` 硬编码）
- 修改：`presentation/panels/simulation/chart_export_utils.py`（若 UI 侧 `ChartSpec` 构造与导出枚举有公共部分，提炼成唯一生成位点）
- 修改：`domain/llm/agent/agent_prompt_builder.py`（所有关于 signal 精读的 guideline 统一落到 `read_signals`，不再提独立 chart tool）

#### 清理清单

- **字面量级删除**：`analysis_chart_viewer.py` 里的 `chart_index = 1` 硬编码；grep 自证零命中
- **整段删除**"UI 当前选中就是 agent 可见全部"的所有隐含假设，以及基于 `self._current_spec` / `self.current_chart_index` 决定"导出哪张"的所有分支
- **整段删除** `export_bundle` 内直接拼 `charts.json` manifest 的自我耦合逻辑——manifest 的构造必须全部委托给 exporter / 共享 util 的公共入口
- **禁止重复** ChartSpec 枚举逻辑：UI 切换用的 spec 列表与 chart 导出枚举必须来自同一个 util；grep 自证 `ChartSpec` / `chart_specs_for` 等生成位点仅一处
- **禁止把 charts 当成 headless 必备工件**：任何成功完成语义、agent bundle 完整性检查、`run_simulation` 返回路径都不得再要求 `charts/` 存在
- **字面量级删除**：文档、prompt、测试计划里所有把 agent chart 读取写成独立 tool 的条目

#### 输出

- chart 导出回到"display artifact，但一旦存在就对 `read_signals` 可发现且全量一致"的正确定位
- agent 对信号的心智模型保持单一：`read_signals` = 唯一入口，chart 只是它的一个可选 source
...
- **成功 job 的完成条件**：`result.json` + 无头权威工件（metrics / raw_data / output_log / analysis_info / `op_result` 若适用）落盘成功即可进入 COMPLETED；`charts/` / `waveforms/` / `current_*.png` 都是 display artifact，**不得**成为 `run_simulation` 或 manager 成功判定的前置条件
...
- **display artifact 失败隔离**：若 UI chart / waveform 导出或 conversation snapshot 写入失败，只影响对应 display/export 路径，不得回写 job 为 FAILED，更不得阻塞 `read_metrics` / `read_signals(raw)` / `read_output_log` 这些权威 read 路径
...
- **禁止把 display artifact 写进成功判定**：manager / service / tool 内任何 `COMPLETED` 条件都不得要求 `charts/` / `waveforms/` / `current_chart.png` / `current_waveform.png` 已存在
...
- **禁止在 `SIM_COMPLETE` 前等待 UI 导出**：发布完成事件前不得等待 chart viewer / waveform widget / attachment snapshot 的额外 I/O
- 执行 grep 自证：`SIM_ERROR` 事件 payload 构造处的 `result_path=` 参数从不为 `None` 字面量

输出：

- 失败与取消路径语义明确；并发路径不互相污染；headless 成功语义不再被 display artifact 污染
...
- 新增测试覆盖：
  ...
  - `read_signals` 的默认 `source='auto'` 始终解析到 raw；`signal_filter` 只做列裁剪；`details` 与 content 都带 `sources_available`
  - `read_signals(source='chart', chart_index=...)` 在 `charts.json` 存在时能精确读到对应 chart CSV；chart 缺失或 index 不存在时 is_error 且列出可用 source / index
  - `read_signals` 的返回 content 永不含 base64；统计列齐全；`.ac` 分析引用 `metrics.json` 而不在自己里面重算 bandwidth / gain margin / phase margin；`waveforms/waveform.csv` 不出现在 agent source 枚举中
  - `read_output_log` 默认返回含 `errors` 全量 + `warnings` 前 20 条 + `tail` 20 行；`section=all` 触发 `truncate_head` 兜底
  - `read_op_result` 对非 `.op` 分析明确报错；文本工件缺失时回退 `result.data.op_result`；返回 content 只含格式化表格不含原始结构
  - Chart 导出多图落盘：对 `.ac` 分析的 fixture 触发一次 display/export，`charts/` 下应生成多条 `<index>_<type>.{png,csv,json}`，`charts.json` manifest 条目数 = 理论 chart spec 数；`read_signals` 可按 `chart_index` 精读每一条；`analysis_chart_viewer.py` 不再出现 `chart_index = 1` 字面量
  - headless 成功路径的 fixture 不要求 `charts/` / `waveforms/` 存在；`run_simulation` 完成后 `read_metrics` / `read_signals(source='raw')` / `read_output_log` 仍可工作
...
- 旧设计清除 grep 清单（合并前必须零命中，或命中点有明确注释解释为何保留）：
  ...
  - `domain/llm/agent/tools/` 下旧的独立波形 / 图表 tool 文件或对应注册条目（物理消失）
  - `agent_prompt_builder.py` / `tool_factory.py` / 本文档后半段里把 agent signal 读取写成多个独立波形 / 图表入口的条目
  - `domain/llm/agent/tools/read_*.py` 内自拼 `<stem>/<ts>/<category>/` 路径字符串或 `get_latest` 直调

涉及文件：

- 新建测试：`tests/test_simulation_job_manager.py`、`tests/test_run_simulation_tool.py`、`tests/test_simulation_tab_job_routing.py`、`tests/test_circuit_selection_view.py`、`tests/test_simulation_artifact_reader_base.py`、`tests/test_read_metrics_tool.py`、`tests/test_read_signals_tool.py`、`tests/test_read_output_log_tool.py`、`tests/test_read_op_result_tool.py`、`tests/test_chart_multi_export.py`
...
- 新架构关键行为有回归测试保护
- 旧的分裂 signal 读取设计在仓库与文档里都不存在——不是"被标记废弃"、不是"被包在兼容层里"

---

## 稳定的 Artifact 布局 Schema（长期合约）

以下清单视为项目长期合约，后续所有 artifact 相关的读写都以此为准；不再允许在其他模块里重新硬编码相对路径字符串。所有文件都已带"电路关联 header"（文本类为 `# circuit_file / # file_path / # analysis_type / # executor / # timestamp` 行；PNG 为等价 tEXt chunks），read tool 可据此自证来源；`result.json` 与子目录共根存放。第 0 步保障的是**无头权威工件**必定存在；display artifact 只在显式导出 / 附件链路触发时附加出现：
...
- `simulation_results/<stem>/<ts>/analysis_info/analysis_info.txt`、`.../analysis_info.json`：分析命令与参数
- `simulation_results/<stem>/<ts>/raw_data/raw_data.csv`、`.../raw_data.json`：原始采样数据
- `simulation_results/<stem>/<ts>/op_result/op_result.txt`、`.../op_result.json`：仅 .op 分析
- `simulation_results/<stem>/<ts>/export_manifest.json`：bundle 内所有文件清单（由 `SimulationArtifactPersistence` 自动生成）

可选 display artifact（存在时供 UI / 附件链路 / `read_signals(source='chart')` 使用）：

- `simulation_results/<stem>/<ts>/charts/<index>_<name>.{png,csv,json}`：display/export 触发后落盘的 chart 视图；`read_signals` 可按 `chart_index` 精确读取对应 CSV
- `simulation_results/<stem>/<ts>/charts/charts.json`：chart manifest，含所有 chart 的 `index / type / title / files` 条目，是 `read_signals` 发现 chart source 的入口
- `simulation_results/<stem>/<ts>/waveforms/waveform.png`、`.../waveform.csv`、`.../waveform.json`：UI overlay 波形导出；**不是** agent 的权威信号源
- `simulation_results/<stem>/<ts>/charts/current_chart.png`、`waveforms/current_waveform.png`：对话附件 / 当前视图 snapshot 固定路径
...
- `SimulationArtifactPersistence` / `run_simulation` / manager 完成判定路径里直接依赖 `_chart_viewer.export_bundle` / `_waveform_widget.export_bundle`
...
- 旧的独立波形 / 图表 agent tool 的实现、注册、prompt guideline、测试计划条目
- `read_signals.py` 内 `waveforms_paths(` 调用或把 `waveforms/waveform.csv` 暴露为 agent source 的分支
...
- 任何把 PNG / 完整波形 base64 塞进 LLM 上下文的 read 路径（`read_signals` 永远只给路径字符串 + 数值摘要，不恢复旧的独立波形 / 图表 read tool）
...
- **磁盘单树**：用户手动或 agent 后台跑完任意一次仿真，磁盘上只在 `<project_root>/simulation_results/<stem>/<ts>/` 下看到 `result.json + metrics/ + output_log/ + analysis_info/ + raw_data/ + op_result/(可选) + export_manifest.json` 这组**无头权威工件**；`.circuit_ai/sim_results/` 目录不再被创建。`charts/` / `waveforms/` 仅在 display/export / 附件链路触发后可选出现
- **Agent bundle 完整性**：用 pytest fixture 在 UI 未加载的情况下触发 `run_simulation` tool，完成后 `read_metrics` / `read_signals(source='raw')` / `read_output_log` / `read_op_result`（若 `.op`）都能直接工作；这组能力**不依赖** `charts/` / `waveforms/`
- **Bundle 结构向后兼容**：Step 0 改造前后各跑一遍基线仿真，先对无头权威工件集合做 `find . -type f | sort`；除新增 `result.json` 外，两次 mandatory set 完全一致。再单独触发 display/export / 附件链路，断言 `charts/<index>_<type>.{png,csv,json}`、`charts/charts.json`、`charts/current_chart.png`、`waveforms/waveform.{png,csv,json}`、`waveforms/current_waveform.png` 的命名与固定路径契约不变
...
- **跨 job 数据零泄漏**：`read_metrics` / `read_signals` / `read_output_log` / `read_op_result` 每次执行只覆盖一个 `result_path` 对应的那一次仿真；在并发多电路 fixture 上反复调用各 read tool，跨电路数据不泄漏
- **多 chart 完整可寻址**：在存在 chart 导出的 bundle 上，`read_signals(source='chart')` 能看到一次 `.ac` 仿真产出的所有 chart spec（bode / phase / ... 至少两类），而不是只有"UI 最后选过的那张"
