...
export function RagPanel({ state, actions, available }: RagPanelProps) {
  ...
  useEffect(() => {
    setQuery('')
    setIsSearchViewActive(Boolean(rag.search.result_text))
    previousResultTextRef.current = rag.search.result_text
  }, [state.context_id])
  ...
  const hasContext = state.context_id.length > 0
  const canSearch = available && hasContext && rag.actions.can_search && query.trim().length > 0 && !rag.search.is_running
    ...
    <div className="rag-shell">
      ...
      <div className="rag-toolbar">
        <button
          ...
          onClick={() => actions?.requestReindexKnowledge(state.context_id)}
          disabled={!available || !hasContext || !rag.actions.can_reindex}
        >
        ...
        <button
          ...
          onClick={() => actions?.requestClearKnowledge(state.context_id)}
          disabled={!available || !hasContext || !rag.actions.can_clear}
        >
          ...
          {isSearchViewActive ? (
            <div className="rag-search-actions">
              <button
                ...
                onClick={() => actions?.requestRagSearch(state.context_id, query.trim())}
                disabled={!canSearch}
              >
      ...
      <div className="rag-files-card">
        ...
        <div className="rag-file-table">
          ...
          <div className="rag-file-table__body">
            {rag.files.length ? (
              rag.files.map((file) => {
                const canOpen = Boolean(file.relative_path)
                return (
                  <button
                    ...
                    onClick={() => {
                      if (canOpen) {
                        actions?.openFile(state.context_id, file.relative_path)
                      }
                    }}
                    disabled={!canOpen}
