...
class RawDataTable(QWidget):
    ...
    def get_web_snapshot(
        ...
        *,
        ...
        if not include_table_data or snapshot is None:
            return {
                "has_data": bool(snapshot is not None and self._model.total_rows > 0),
                "columns": [],
                "rows": [],
            }
        columns = [self._model.x_label, *self._model.signal_names]
        column_arrays = []
        column_arrays.append(snapshot.x_values)
        column_arrays.extend(snapshot.signal_columns.get(signal_name) for signal_name in self._model.signal_names)
        rows = []
        for row_index in range(self._model.total_rows):
            values = [
                self._format_web_value(column_values, row_index)
                for column_values in column_arrays
            ]
            rows.append({
        ...
        return {
            "has_data": bool(self._model.total_rows > 0),
            "columns": columns,
            "rows": rows,
        }
