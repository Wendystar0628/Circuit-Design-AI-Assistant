...
以下属于新架构上的功能扩展，未作为本轮已实现能力宣称。

| 优先级 | 建议 | 完成判据 |
|---|---|---|
| P1 | 将测量目标从自由文本升级为结构化约束：指标、单位、上下限、工况 | 能区分 PASS / FAIL / 未测得，失败测量不可当零值通过；历史验收条件随实验保存 |
| P1 | 批量实验与参数、温度、供电、负载 corner 矩阵 | 每个 case 有独立输入与结果身份、取消与失败状态，汇总最坏情况和指标差值 |
| P1 | 数值可信度实验：容差/最大步长加密复算 | 指标变化低于用户设定阈值才标记稳定，同时保留成本与两次结果 |
| P1 | 模型管理：来源、版本、适用电压/频率/温度、简化模型标识 | 用户能知道采用的模型及假设；模型升级对历史输入无影响 |
