...
from domain.llm.conversation_rollback_service import MAX_ROLLBACK_CHECKPOINTS, ConversationRollbackService
from domain.llm.session_state_manager import SessionStateManager
from domain.llm.working_context_builder import WORKING_CONTEXT_COMPRESSED_COUNT_KEY, WORKING_CONTEXT_KEEP_RECENT_KEY, WORKING_CONTEXT_SUMMARY_KEY
...
from shared.service_names import SVC_CONTEXT_MANAGER, SVC_EVENT_BUS, SVC_FILE_MANAGER, SVC_FILE_WATCHER, SVC_SESSION_STATE_MANAGER
...
def _prepare_checkpointed_conversation(tmp_path: Path):
    ...
    pending_edits_base = {'files': [{'path': 'design.txt', 'added_lines': 1}], 'updated': 'before-anchor'}
    pending_edits_path.write_text(json.dumps(pending_edits_base), encoding='utf-8')
    context_manager.add_user_message('base question', timestamp='2026-04-08T19:00:00', message_id='base-user')
    context_manager.add_assistant_message('base answer', timestamp='2026-04-08T19:00:10', message_id='base-assistant')
    metadata_before_anchor = {WORKING_CONTEXT_SUMMARY_KEY: 'summary before anchor', WORKING_CONTEXT_COMPRESSED_COUNT_KEY: 1, WORKING_CONTEXT_KEEP_RECENT_KEY: 1, 'circuit_file_path': 'before-anchor.cir', 'last_metrics': {'gain': 10.0}, 'error_context': 'before-anchor error context'}
    context_manager.get_current_state().update(copy.deepcopy(metadata_before_anchor))
    manager.mark_dirty()
    asyncio.run(rollback_service.capture_user_turn_checkpoint(anchor_message_id='anchor-user', anchor_timestamp='2026-04-08T19:01:00'))
    workspace_file.write_text('base-1\nchanged-2\n', encoding='utf-8')
    new_workspace_file = tmp_path / 'after-anchor.txt'
    new_workspace_file.write_text('temp-1\ntemp-2\n', encoding='utf-8')
    pending_edits_path.write_text(json.dumps({'files': [], 'updated': 'after-anchor'}), encoding='utf-8')
    pending_edits_after_anchor = {'files': [], 'updated': 'after-anchor'}
    long_user_message = 'anchor ' + 'very long message ' * 20
    context_manager.add_user_message(long_user_message, timestamp='2026-04-08T19:01:00', message_id='anchor-user')
    context_manager.add_assistant_message('assistant after anchor', timestamp='2026-04-08T19:01:10', message_id='after-assistant')
    metadata_after_anchor = {WORKING_CONTEXT_SUMMARY_KEY: 'summary after anchor', WORKING_CONTEXT_COMPRESSED_COUNT_KEY: 3, WORKING_CONTEXT_KEEP_RECENT_KEY: 2, 'circuit_file_path': 'after-anchor.cir', 'last_metrics': {'gain': 20.0}, 'error_context': 'after-anchor error context'}
    context_manager.get_current_state().update(copy.deepcopy(metadata_after_anchor))
    manager.mark_dirty()
    return {'manager': manager, 'context_manager': context_manager, 'file_watcher': file_watcher, 'rollback_service': rollback_service, 'session_id': session_id, 'workspace_file': workspace_file, 'new_workspace_file': new_workspace_file, 'pending_edits_path': pending_edits_path, 'pending_edits_base': pending_edits_base, 'pending_edits_after_anchor': pending_edits_after_anchor, 'long_user_message': long_user_message, 'metadata_before_anchor': metadata_before_anchor, 'metadata_after_anchor': metadata_after_anchor}
...
def test_preview_rollback_to_anchor_exposes_workspace_changes_and_message_truncation(tmp_path: Path):
    ...
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
    ...
    current_session_path = f'{context_service.CONVERSATIONS_DIR}/{env['session_id']}.json'
    current_rollback_path = f'{context_service.CONVERSATIONS_DIR}/{env['session_id']}{context_service.ROLLBACK_CHECKPOINTS_SUFFIX}'
    current_metadata_path = f'{context_service.CONVERSATIONS_DIR}/{context_service.SESSIONS_INDEX_FILE}'
    assert preview.changed_file_count == 5
    assert workspace_paths == [current_session_path, current_rollback_path, current_metadata_path, 'after-anchor.txt', 'design.txt']
    assert all((path in {current_session_path, current_rollback_path, current_metadata_path} for path in workspace_paths if path.startswith(f'{SYSTEM_DIR}/')))
...
def test_rollback_to_anchor_restores_workspace_and_session_state(tmp_path: Path):
    ...
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
    result = asyncio.run(env['rollback_service'].rollback_to_anchor('anchor-user', expected_operation_token=preview.operation_token))
    ...
    persisted_ids = [msg.get('additional_kwargs', {}).get('metadata', {}).get('id', '') for msg in persisted_messages]
    ...
    assert [getattr(message, 'content', '') for message in runtime_messages] == ['base question', 'base answer']
    persisted_metadata = context_service.get_session_metadata(str(tmp_path), env['session_id'])
    ...
    assert [os.path.normcase(path) for path in env['file_watcher'].start_calls] == [os.path.normcase(str(tmp_path.resolve()))]

def test_rollback_confirmation_rejects_workspace_changes_after_preview(tmp_path: Path):
    env = _prepare_checkpointed_conversation(tmp_path)
    env['file_watcher'].is_watching = True
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
...
def test_project_switch_during_restore_cannot_commit_or_rebind_watcher(tmp_path: Path, monkeypatch):
    env = _prepare_checkpointed_conversation(tmp_path)
    manager = env['manager']
    ...
    preview = asyncio.run(rollback_service.preview_rollback_to_anchor('anchor-user'))
    ...
    monkeypatch.setattr('domain.llm.conversation_rollback_service.snapshot_service.restore_snapshot_async', blocked_restore)
    monkeypatch.setattr(manager, 'reload_current_session', lambda *args, **kwargs: reload_calls.append((args, kwargs)))
    monkeypatch.setattr(rollback_service, '_publish_workspace_sync_required', lambda **kwargs: workspace_sync_calls.append(kwargs))

    async def scenario():
        task = asyncio.create_task(rollback_service.rollback_to_anchor('anchor-user', expected_operation_token=preview.operation_token))
    ...
    assert [os.path.normcase(path) for path in file_watcher.start_calls] == [os.path.normcase(project_b)]
...
def test_rollback_owner_rejects_same_project_after_generation_rebind(tmp_path: Path):
    ...
    owner = service._capture_owner(manager, session_id=session_id, project_root=str(tmp_path))
...
def test_conversation_rollback_preserves_other_sessions_and_global_index(tmp_path: Path):
    ...
    context_service.save_rollback_checkpoints(str(tmp_path), other_session_id, other_checkpoints)
    context_service.update_session_index(str(tmp_path), other_session_id, {'name': 'Created after checkpoint', 'marker': 'keep-me'})
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
    ...
    assert f'{context_service.CONVERSATIONS_DIR}/{other_session_id}{context_service.ROLLBACK_CHECKPOINTS_SUFFIX}' not in preview_paths
    assert f'{context_service.CONVERSATIONS_DIR}/{context_service.SESSIONS_INDEX_FILE}' in preview_paths
    result = asyncio.run(env['rollback_service'].rollback_to_anchor('anchor-user', expected_operation_token=preview.operation_token))
    assert result['success'] is True
    assert context_service.load_messages(str(tmp_path), other_session_id) == other_messages
    assert context_service.load_rollback_checkpoints(str(tmp_path), other_session_id) == other_checkpoints
...
def test_session_metadata_commit_failure_restores_preconfirm_state(tmp_path: Path, monkeypatch):
    env = _prepare_checkpointed_conversation(tmp_path)
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
    live_metadata = copy.deepcopy(context_service.get_session_metadata(str(tmp_path), env['session_id']))
    live_message_ids = [message.id for message in env['context_manager'].get_display_messages()]
    live_workspace = env['workspace_file'].read_text(encoding='utf-8')
    monkeypatch.setattr(context_service, 'replace_session_metadata', lambda *args, **kwargs: False)
    result = asyncio.run(env['rollback_service'].rollback_to_anchor('anchor-user', expected_operation_token=preview.operation_token))
    assert result['success'] is False
    ...
    assert [message.id for message in env['context_manager'].get_display_messages()] == live_message_ids
    assert context_service.get_session_metadata(str(tmp_path), env['session_id']) == live_metadata
    runtime_state = env['context_manager'].get_current_state()
    for key, expected_value in env['metadata_after_anchor'].items():
        assert runtime_state[key] == expected_value

def test_reload_failure_after_metadata_commit_recovers_files_and_metadata(tmp_path: Path, monkeypatch):
    env = _prepare_checkpointed_conversation(tmp_path)
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
    live_metadata = copy.deepcopy(context_service.get_session_metadata(str(tmp_path), env['session_id']))
    live_workspace = env['workspace_file'].read_text(encoding='utf-8')
    ...
    monkeypatch.setattr(env['manager'], 'reload_current_session', fail_first_reload)
    result = asyncio.run(env['rollback_service'].rollback_to_anchor('anchor-user', expected_operation_token=preview.operation_token))
    assert result['success'] is False
...
def test_concurrent_metadata_change_during_commit_fails_closed_and_recovers_files(tmp_path: Path, monkeypatch):
    env = _prepare_checkpointed_conversation(tmp_path)
    preview = asyncio.run(env['rollback_service'].preview_rollback_to_anchor('anchor-user'))
    live_workspace = env['workspace_file'].read_text(encoding='utf-8')
    ...
    def inject_concurrent_change(*args, **kwargs):
        ...
        replacement_calls += 1
        assert context_service.update_session_index(str(tmp_path), env['session_id'], {WORKING_CONTEXT_SUMMARY_KEY: 'concurrent summary'})
        return original_replace(*args, **kwargs)
    monkeypatch.setattr(context_service, 'replace_session_metadata', inject_concurrent_change)
    result = asyncio.run(env['rollback_service'].rollback_to_anchor('anchor-user', expected_operation_token=preview.operation_token))
    assert result['success'] is False
    assert result['plan_changed'] is True
    ...
    assert env['workspace_file'].read_text(encoding='utf-8') == live_workspace
    assert env['new_workspace_file'].exists()
    actual_metadata = context_service.get_session_metadata(str(tmp_path), env['session_id'])
    assert actual_metadata is not None
    assert actual_metadata[WORKING_CONTEXT_SUMMARY_KEY] == 'concurrent summary'
    assert env['context_manager'].get_current_state()[WORKING_CONTEXT_SUMMARY_KEY] == 'concurrent summary'

def test_watcher_stop_failure_does_not_restore_over_new_workspace_edit(tmp_path: Path, monkeypatch):
    env = _prepare_checkpointed_conversation(tmp_path)
    env['file_watcher'].is_watching = True
...
def test_project_open_event_ensures_active_session_and_syncs_context(tmp_path: Path):
    ...
    ServiceLocator.register(SVC_SESSION_STATE_MANAGER, manager)
    event_bus.publish(EVENT_STATE_PROJECT_OPENED, {'path': str(tmp_path)}, source='test')
    ...
    assert os.path.normcase(current_state.get('project_root', '')) == os.path.normcase(str(tmp_path.resolve()))
...
def test_capture_user_turn_checkpoint_uses_project_opened_active_session(tmp_path: Path):
    ...
    rollback_service = ConversationRollbackService()
    event_bus.publish(EVENT_STATE_PROJECT_OPENED, {'path': str(tmp_path)}, source='test')
    checkpoint = asyncio.run(rollback_service.capture_user_turn_checkpoint(anchor_message_id='anchor-user', anchor_timestamp='2026-04-10T16:51:49'))
    checkpoints = context_service.load_rollback_checkpoints(str(tmp_path), manager.get_current_session_id())
...
def test_checkpoint_capture_keeps_latest_twenty_without_deleting_other_session(tmp_path: Path, monkeypatch):
    manager, _, _, session_id = _build_environment(tmp_path)
    service = ConversationRollbackService()
    snapshots_dir = Path(snapshot_service.get_snapshots_dir(str(tmp_path)))

    async def create_empty_snapshot(project_root, snapshot_id, **kwargs):
        del project_root, kwargs
        (snapshots_dir / snapshot_id).mkdir(parents=True)
        return f'{snapshot_service.SNAPSHOTS_DIR}/{snapshot_id}'
    monkeypatch.setattr(snapshot_service, 'create_snapshot_async', create_empty_snapshot)
    other_snapshot_id = service._get_session_snapshot_prefix('another-session') + 'must_survive'
    (snapshots_dir / other_snapshot_id).mkdir(parents=True)
    captured_snapshot_ids = []
    for index in range(MAX_ROLLBACK_CHECKPOINTS + 1):
        checkpoint = asyncio.run(service.capture_user_turn_checkpoint(anchor_message_id=f'anchor-{index:02d}', anchor_timestamp=f'2026-04-10T16:51:{index:02d}'))
        captured_snapshot_ids.append(checkpoint['snapshot_id'])
    checkpoints = context_service.load_rollback_checkpoints(str(tmp_path), session_id)
    assert [item['anchor_message_id'] for item in checkpoints] == [f'anchor-{index:02d}' for index in range(1, MAX_ROLLBACK_CHECKPOINTS + 1)]
    assert not (snapshots_dir / captured_snapshot_ids[0]).exists()
    assert all(((snapshots_dir / snapshot_id).is_dir() for snapshot_id in captured_snapshot_ids[1:]))
    assert (snapshots_dir / other_snapshot_id).is_dir()

def test_checkpoint_capture_stops_before_snapshot_when_session_save_fails(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match='persist current conversation'):
        asyncio.run(service.capture_user_turn_checkpoint(anchor_message_id='anchor-save-failed', anchor_timestamp='2026-04-10T16:51:49'))
    assert snapshot_calls == []
    assert context_service.load_rollback_checkpoints(str(tmp_path), manager.get_current_session_id()) == []

def test_project_switch_during_checkpoint_capture_cannot_register_stale_anchor(tmp_path: Path, monkeypatch):
    ...
    async def scenario():
        ...
        async def blocked_snapshot(project_root, snapshot_id, **kwargs):
            result = await original_create_snapshot(project_root, snapshot_id, **kwargs)
            snapshot_created.set()
            await allow_return.wait()
            return result
        monkeypatch.setattr(snapshot_service, 'create_snapshot_async', blocked_snapshot)
        task = asyncio.create_task(service.capture_user_turn_checkpoint(anchor_message_id='anchor-project-a', anchor_timestamp='2026-04-10T16:51:49'))
...
def test_checkpoint_persistence_failure_removes_its_unique_snapshot(tmp_path: Path, monkeypatch):
    manager, _, _, session_id = _build_environment(tmp_path)
    service = ConversationRollbackService()
    monkeypatch.setattr(context_service, 'append_rollback_checkpoint', lambda *args, **kwargs: False)
    with pytest.raises(RuntimeError, match='persist rollback checkpoint'):
        asyncio.run(service.capture_user_turn_checkpoint(anchor_message_id='anchor-checkpoint-failed', anchor_timestamp='2026-04-10T16:51:49'))
    assert context_service.load_rollback_checkpoints(str(tmp_path), session_id) == []
    snapshots_dir = Path(tmp_path) / snapshot_service.SNAPSHOTS_DIR
    assert not snapshots_dir.exists() or not any(snapshots_dir.iterdir())
...
def test_checkpoint_snapshot_prefix_uses_full_session_identity():
    ...
    assert service._get_session_snapshot_prefix(f'{shared_prefix}aaaaaaaa') != service._get_session_snapshot_prefix(f'{shared_prefix}bbbbbbbb')

@pytest.mark.parametrize('invalid_snapshot_id', ['../../outside-snapshot', '.', '..', 'name.with-dot', '������'])
@pytest.mark.parametrize('loader_name', ['_load_snapshot_session_messages', '_load_snapshot_session_metadata'])
def test_snapshot_session_readers_reject_noncanonical_checkpoint_identity(tmp_path: Path, invalid_snapshot_id: str, loader_name: str):
    service = ConversationRollbackService()
    loader = getattr(service, loader_name)
    with pytest.raises(RuntimeError, match='Invalid rollback snapshot ID'):
        loader(str(tmp_path), invalid_snapshot_id, 'session-preview')
    with pytest.raises(RuntimeError, match='Invalid conversation session ID'):
        loader(str(tmp_path), 'safe-snapshot', '../outside-session')
