...
def test_parser_obeys_title_control_and_end_boundaries() -> None:
    content = (
        "R title that must not become a resistor\n"
...
def test_parser_unifies_ngspice_gnd_alias_with_node_zero() -> None:
    document = SpiceParser().parse_content(
        _deck("R1 OUT GND 1k\nR2 out 0 2k"),
        "ground_alias.cir",
    )
...
def test_subcircuit_params_are_not_misreported_as_ports() -> None:
    content = _deck(
        ".subckt FILTER in out PARAMS: gain=2 corner=1k\n"
        "R1 in out 1k\n"
        ".ends FILTER\n"
...
def test_four_terminal_bjt_does_not_require_model_registry_guessing() -> None:
    document = SpiceParser().parse_content(
        _deck("Q1 collector base emitter substrate ExternalVendorModel"),
        "external_bjt.cir",
    )
...
def test_inline_opamp_v_supply_names_are_power_rails_not_signal_inputs() -> None:
    content = _deck(
        ".subckt IDEAL_OPAMP in+ in- out v+ v-\n"
        ".ends IDEAL_OPAMP\n"
    ...
    assert document.components[0].pin_roles == {
        "plus": "input_plus",
        "minus": "input_minus",
        "out": "output",
        "vcc": "power_positive",
...
def test_source_graph_rejects_ambiguous_active_model_definitions(tmp_path) -> None:
    ...
    first_library.write_text(
        ".model SHARED_DEVICE NMOS(level=1)\n",
        encoding="utf-8",
    )
    second_library.write_text(
        ".model SHARED_DEVICE PMOS(level=1)\n",
        encoding="utf-8",
    )
    ...
    assert any(
        "shared_device" in error.message.lower()
        and "重复定义" in error.message
        for error in document.parse_errors
    )


def test_active_include_components_share_the_callers_electrical_scope(
    tmp_path,
) -> None:
    ...
    assert {
        ...
        for connection in supply_nets[0]["connections"]
    } == {"V1", "RLOAD"}
