...
> **设计原则**：时间戳显示直接集成到 `MetricsSummaryPanel` 中，无需独立组件

- [X] **更新 `MetricsSummaryPanel`**：
  - 在顶部添加时间戳容器 `_timestamp_bar: QFrame`
  - 时间戳容器包含图标 `_timestamp_icon: QLabel` 和文本 `_timestamp_label: QLabel`
...
- [X] **UI 设计**：
  - 背景色：略深于面板背景（`#e8e9ea`）
  - 高度：28px
  - 图标：`resources/icons/simulation/clock.svg`
  - 文本格式："仿真时间：2026-01-06 14:30:22"
  - 字体：`FONT_SIZE_SMALL`，颜色 `COLOR_TEXT_SECONDARY`
- [X] **更新 `SimulationTab`**：
  ...
  - 在 `_show_empty_state()` 中隐藏时间戳栏
  - 在 `_hide_empty_state()` 中显示时间戳栏
