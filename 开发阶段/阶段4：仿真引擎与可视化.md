...
│   ├── path_preprocessor.py         # 路径预处理器（处理用户网表中的路径）
...
│       │         │    [原始仿真输出]                                      │
│       │         │                                                       │
│       │         ├──► SpiceErrorParser.parse() ──► [错误? → 恢复策略]    │
│       │         │                                                       │
│       │         ▼                                                       │
...
- [ ] **被调用方**：`SpiceErrorParser`、`SimulationResult`、`FixErrorAction`
...
- [ ] **核心功能**：
  ...
  - `parse_includes(file_path) -> list[IncludeInfo]` - 解析文件中的 `.include` 和 `.lib` 语句
...
- [ ] **被调用方**：`simulation_service.py`（仿真服务）、`path_preprocessor.py`（路径预处理）
...
- [ ] **被调用方**：`SpiceExecutor`、`PathPreprocessor`、`LibraryManager`
...
> 
> **职责精简说明**：根据单一职责原则，ngspice 路径配置委托给 `ngspice_config`，错误解析委托给 `SpiceErrorParser`，错误恢复委托给 `SpiceErrorRecovery`，路径预处理委托给 `PathPreprocessor`，本模块专注于仿真执行核心逻辑

- [ ] **文件路径**：`domain/simulation/executor/spice_executor.py`
- [ ] **职责**：实现 `SimulationExecutor` 接口，封装 PySpice 库执行 SPICE 电路仿真
- [ ] **继承**：`SimulationExecutor`（抽象基类）
...
- [ ] **仿真执行流程**：
  1. 检查 `ngspice_config.is_ngspice_available()` 确保 ngspice 已配置（配置在 main.py 启动时完成）
  2. 调用 `PathPreprocessor.preprocess()` 处理用户网表中的 .include/.lib 路径
  3. 检查预处理结果中的 `missing_files` 列表
  4. 如果存在缺失文件，返回 `SimulationError` 并提示用户
  5. 使用 PySpice 的 `NgSpiceShared` 加载处理后的网表并执行仿真（共享库模式，同进程内执行）
  6. 仿真完成后调用 `PathPreprocessor.cleanup()` 清理临时文件
  7. 如果仿真失败，调用 `SpiceErrorParser.parse()` 解析错误
  8. 如果错误可恢复，调用 `SpiceErrorRecovery.apply_recovery()` 尝试恢复
  9. 返回仿真结果
- [ ] **依赖**：
  - `PySpice` 库 - SPICE 仿真引擎封装（通过 NgSpiceShared 共享库模式调用 ngspice）
  - `infrastructure.utils.ngspice_config` - ngspice 路径配置（已实现）
- [ ] **依赖的子模块**：
  - `PathPreprocessor` - 路径预处理（详见 4.2.6）
  - `SpiceErrorParser` - 错误解析（详见 4.2.6.1）
  - `SpiceErrorRecovery` - 错误恢复策略（详见 4.2.7）
- [ ] **元器件模型集成**（阶段十实现，此处预留接口）：
  - `check_required_models(circuit_file)` - 检查电路所需的 SPICE 模型
  - `_resolve_model_path(model_name)` - 解析模型文件路径
- [ ] **被调用方**：`executor_registry.py`（执行器注册表）

#### 4.2.6 `path_preprocessor.py` - 路径预处理器

> **设计原则**：解决跨平台路径兼容性问题，确保 ngspice 能正确解析 `.include` 和 `.lib` 引用
>
> **职责边界**：仅负责路径解析和临时文件生成，不依赖高层服务，保持模块独立性

- [ ] **文件路径**：`domain/simulation/executor/path_preprocessor.py`
- [ ] **职责**：在仿真执行前预处理电路文件中的路径引用，将相对路径转换为绝对路径，生成临时文件供仿真使用
- [ ] **核心功能**：
  - `preprocess(circuit_file, project_root) -> PreprocessResult` - 预处理电路文件
  - `cleanup(preprocess_result)` - 清理临时文件
  - `_resolve_include_path(raw_path, source_file, project_root) -> str` - 解析单个引用路径
  - `_generate_temp_file(original_file, modified_content) -> Path` - 生成临时文件
  - `_normalize_path_for_ngspice(path) -> str` - 将路径转换为 ngspice 兼容格式
- [ ] **`PreprocessResult` 数据结构**：
  - `original_file: Path` - 原始电路文件路径
  - `temp_file: Path` - 生成的临时文件路径（供仿真使用）
  - `path_mappings: list[PathMapping]` - 路径映射记录（用于错误信息还原）
  - `missing_files: list[MissingFileInfo]` - 缺失文件列表（新增）
  - `warnings: list[str]` - 预处理警告信息
- [ ] **`MissingFileInfo` 数据结构**（新增）：
  - `line_number: int` - 引用语句所在行号
  - `raw_path: str` - 原始路径字符串
  - `resolved_path: str` - 尝试解析的绝对路径
  - `statement_type: str` - 语句类型（"include" 或 "lib"）
- [ ] **`PathMapping` 数据结构**：
  - `line_number: int` - 原始行号
  - `original_path: str` - 原始路径字符串
  - `resolved_path: str` - 解析后的绝对路径
- [ ] **路径解析规则**：
  - 相对路径：基于源文件所在目录解析，然后转换为绝对路径
  - 绝对路径：保持不变，仅做格式规范化
  - 路径分隔符：统一转换为正斜杠（ngspice 在所有平台都支持正斜杠）
  - 空格处理：路径包含空格时使用双引号包裹
- [ ] **临时文件管理**：
  - 临时文件存储在 `.circuit_ai/temp/sim_{uuid}/` 目录
  - 保持原始文件名，便于错误信息定位
  - 仿真完成后自动清理（无论成功或失败）
  - 支持调试模式：设置环境变量 `CIRCUIT_AI_KEEP_TEMP=1` 保留临时文件
- [ ] **递归处理**：
  - 被引用的文件（`.include` 目标）也需要预处理
  - 构建处理队列，避免循环引用导致无限递归
  - 记录已处理文件集合，跳过重复处理
- [ ] **错误处理**：
  - **引用文件不存在**：记录到 `missing_files` 列表，继续处理其他文件，不阻止预处理完成
  - **循环引用**：检测并记录警告，跳过循环引用的文件
  - **路径解析失败**：记录警告，保留原始路径
- [ ] **调用方职责**：
  - 调用方（`SpiceExecutor`）检查 `PreprocessResult.missing_files`
  - 如果存在缺失文件，调用方决定是否继续仿真或报错
  - 建议流程：
    ```python
    preprocess_result = preprocessor.preprocess(circuit_file, project_root)
    
    if preprocess_result.missing_files:
        # 调用方决定如何处理缺失文件
        # 选项1：直接报错
        raise SimulationError("存在缺失的引用文件")
        
        # 选项2：查询依赖健康服务（如果需要）
        # health_report = dependency_service.get_cached_report()
        # ...
    ```
- [ ] **模型库路径解析**：
  - 调用 `get_ngspice_models_path()` 获取模型库基础路径
  - 支持 `models/cmp/`、`models/sub/`、`models/custom/` 三类路径
  - 支持模型库别名语法：`@cmp/standard.bjt`、`@sub/LM741.lib`、`@custom/my_opamp.sub`
  - 自动将相对路径转换为基于 vendor 目录的绝对路径
- [ ] **被调用方**：`spice_executor.py`（仿真执行器）

#### 4.2.6.1 `spice_error_parser.py` - SPICE错误解析器

> **设计原则**：将错误解析逻辑从 spice_executor 中分离，便于复用和测试

- [ ] **文件路径**：`domain/simulation/executor/spice_error_parser.py`
- [ ] **职责**：解析 ngspice 输出，识别错误类型并提取详细信息
- [ ] **核心功能**：
  - `parse(output: str) -> SimulationError` - 解析 ngspice 输出，返回结构化错误
  - `extract_error_line(output)` - 提取错误行号和上下文代码
  - `extract_missing_models(output)` - 提取缺失的模型名称列表
  - `extract_floating_nodes(output)` - 提取浮空节点名称列表
  - `classify_error(output)` - 根据输出特征分类错误类型
- [ ] **错误识别规则**：
  - 语法错误：匹配 "Error on line"、"syntax error" 等关键词
  - 模型缺失：匹配 "Unknown device"、"model not found" 等关键词
  - 节点浮空：匹配 "floating"、"no DC path to ground" 等关键词
  - 收敛失败：匹配 "no convergence"、"timestep too small" 等关键词
  - 超时：由调用方根据进程状态判断
- [ ] **被调用方**：`spice_executor.py`、`fix_error_action.py`

#### 4.2.7 `spice_error_recovery.py` - SPICE错误恢复策略

> **设计原则**：将错误恢复策略从 spice_executor 中分离，支持策略扩展，增加失败保护机制

- [ ] **文件路径**：`domain/simulation/executor/spice_error_recovery.py`
- [ ] **职责**：根据错误类型提供自动恢复策略和用户建议，防止无限恢复循环
- [ ] **核心功能**：
  - `can_auto_recover(error: SimulationError) -> bool` - 判断是否支持自动恢复
  - `get_recovery_strategy(error: SimulationError) -> RecoveryStrategy` - 获取恢复策略
  - `apply_recovery(error, original_config, attempt_count) -> RecoveryResult` - 应用恢复策略（新增 attempt_count 参数）
  - `get_user_suggestion(error: SimulationError) -> str` - 获取用户友好的恢复建议
  - `get_recovery_history() -> list[RecoveryAttempt]` - 获取恢复历史记录（新增）
  - `clear_recovery_history()` - 清空恢复历史（新增）
- [ ] **恢复策略枚举**：
  - `REDUCE_PRECISION` - 降低仿真精度（放宽 gmin、reltol 等）
  - `REDUCE_TIMESTEP` - 减小时间步长
  - `ADD_INITIAL_CONDITIONS` - 添加初始条件
  - `INCREASE_ITERATIONS` - 增加迭代次数限制
  - `NO_AUTO_RECOVERY` - 不支持自动恢复，仅提供建议
- [ ] **恢复限制机制**（新增）：
  - `MAX_RECOVERY_ATTEMPTS = 3` - 最大恢复尝试次数
  - 每次恢复尝试递增 `attempt_count`
  - 达到上限后返回失败，不再尝试
  - 防止无限恢复循环消耗资源
- [ ] **`RecoveryResult` 数据结构**（新增）：
  - `success: bool` - 是否成功生成恢复配置
  - `modified_config: Optional[dict]` - 修改后的配置（成功时）
  - `strategy_applied: Optional[RecoveryStrategy]` - 应用的策略
  - `message: str` - 结果消息
  - `should_retry: bool` - 是否应该重试仿真
  - `attempt_count: int` - 当前尝试次数
- [ ] **`RecoveryAttempt` 数据结构**（新增）：
  - `timestamp: str` - 尝试时间
  - `error_type: SimulationErrorType` - 错误类型
  - `strategy: RecoveryStrategy` - 使用的策略
  - `attempt_number: int` - 尝试序号
  - `success: bool` - 是否成功
  - `config_changes: dict` - 配置变更内容
- [ ] **恢复历史记录**（新增）：
  - 每次恢复尝试记录到内存列表
  - 包含时间戳、错误类型、策略、结果
  - 用于调试和用户反馈
  - 仿真成功后可选择性清空历史
- [ ] **用户反馈机制**：
  - **恢复尝试通知**：自动恢复开始时发布 `EVENT_RECOVERY_ATTEMPTING` 事件
    - 事件数据：错误类型、恢复策略、预计影响、尝试次数
    - UI 显示：状态栏提示"正在尝试自动恢复（第 N/3 次）：降低仿真精度..."
  - **恢复结果通知**：恢复完成后发布 `EVENT_RECOVERY_RESULT` 事件
    - 成功：显示"自动恢复成功，仿真继续"
    - 失败：显示"自动恢复失败（已尝试 3 次），请查看错误详情"
  - **恢复历史记录**：记录每次恢复尝试到仿真日志
    - 包含：原始错误、尝试的策略、结果、耗时、配置变更
  - **用户干预选项**：
    - 可配置"禁用自动恢复"，所有错误直接报告给用户
    - 可配置"恢复前确认"，自动恢复前弹窗询问用户
- [ ] **恢复策略应用示例**：
  ```python
  # 收敛失败 → 降低精度
  if error.type == SimulationErrorType.CONVERGENCE_DC:
      modified_config = {
          "gmin": original_config["gmin"] * 10,      # 放宽 10 倍
          "reltol": original_config["reltol"] * 2,   # 放宽 2 倍
          "itl1": original_config["itl1"] + 50       # 增加迭代次数
      }
  ```
- [ ] **被调用方**：`spice_executor.py`

#### 4.2.8 `python_executor.py` - Python脚本仿真执行器
...
- [ ] **文件路径**：`domain/simulation/executor/python_executor.py`
- [ ] **职责**：实现 `SimulationExecutor` 接口，在隔离子进程中执行用户自定义的 Python 仿真脚本
- [ ] **继承**：`SimulationExecutor`（抽象基类）
- [ ] **支持的扩展名**：`.py`
- [ ] **适用场景**：
...
#### 4.2.9 `file_discovery.py` - 文件发现策略
