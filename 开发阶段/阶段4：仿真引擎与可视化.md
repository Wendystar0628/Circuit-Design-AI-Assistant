...
- [x] **仿真执行流程**：
  ...
  3. 加载电路文件内容
  4. 使用 PySpice 的 SpiceParser 解析网表
  5. 根据分析类型执行仿真（AC/DC/瞬态/噪声/工作点）
  6. 提取仿真数据到标准化 SimulationData 结构
  7. 如果仿真失败，解析异常并返回结构化错误
  8. 返回 SimulationResult
  - **注**：PathPreprocessor、SpiceErrorParser、SpiceErrorRecovery 集成待后续实现
- [x] **依赖**：
