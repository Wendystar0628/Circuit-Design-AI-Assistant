...
> 
> **职责精简说明**：根据单一职责原则，ngspice 路径配置委托给 `ngspice_config`，错误恢复委托给 `SpiceErrorRecovery`，错误解析作为执行器内部职责（`_parse_exception` 方法），本模块专注于仿真执行核心逻辑
>
...
- [ ] **依赖的子模块**：
  - `SpiceErrorRecovery` - 错误恢复策略（详见 4.2.6）
...
#### 4.2.6 `spice_error_recovery.py` - SPICE错误恢复策略
