...
├── service/                         # 仿真服务层（简化后 2 个模块）
│   ├── __init__.py
│   ├── simulation_config_service.py # 配置管理（可选）
│   └── tuning_service.py            # 快速调参（可选）
├── analysis/                        # 高级分析模块
│   ├── __init__.py
│   ├── pvt_analysis.py
...
#### 4.2.8 `circuit_file_scanner.py` - 电路文件扫描器

> **设计简化说明**：原设计采用策略模式支持"自动扫描"和"手动选择"两种文件发现方式，但"手动选择"本质上是 UI 层职责（弹出文件对话框），不应放在 domain 层。简化为单一的扫描工具类。
>
> **实现说明**：扫描功能已整合到 `circuit_analyzer.py` 中，不再单独创建文件。

- [x] **文件路径**：`domain/simulation/executor/circuit_analyzer.py`（整合到现有文件）
- [x] **职责**：扫描项目目录中的可仿真文件，识别主电路候选
- [x] **核心功能**：
  - `scan_simulatable_files(project_path) -> ScanResult` - 扫描项目目录，返回可仿真文件列表和主电路候选
  - `get_supported_extensions() -> list[str]` - 获取支持的文件扩展名（从 executor_registry 获取）
- [x] **`ScanResult` 数据结构**：
  - `files: list[Path]` - 发现的可仿真文件列表
  - `main_circuit_candidates: list[Path]` - 主电路候选列表（可能为 0、1 或多个）
  - `dependency_graph: dict` - 文件依赖关系图
- [x] **扫描逻辑**：
  - 从 `executor_registry.get_all_supported_extensions()` 获取扫描范围
  - 递归扫描项目目录，收集匹配扩展名的文件
  - 调用 `circuit_analyzer.detect_main_circuit()` 分析主电路候选
- [x] **与 UI 层的分工**：
  - 本模块：提供扫描结果数据
  - UI 层：根据扫描结果显示文件列表，或弹出文件选择对话框让用户手动选择
- [x] **被调用方**：`simulation_service.py`（仿真服务）
...
- [ ] **配置存储**：`.circuit_ai/simulation_config.json`
- [ ] **配置文件结构**：
  ```json
  {
    "version": "1.0",
...
- [ ] **事件发布**：`EVENT_SIM_CONFIG_CHANGED` - 配置变更时发布
- [ ] **被调用方**：`SimulationService`、`SimulationConfigDialog`（UI）

#### 4.3.3 主电路检测功能说明

> **实现说明**：主电路检测功能已整合到 `circuit_analyzer.py` 和 `simulation_service.py` 中，不再单独创建 `main_circuit_detector.py` 文件。

- [x] **检测功能位置**：`circuit_analyzer.detect_main_circuit(project_path)`
...
#### 4.3.4 `tuning_service.py` - 快速调参服务（可选）
