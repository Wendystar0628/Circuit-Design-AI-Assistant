...
- [ ] **被调用方**：`SpiceExecutor`、`LibraryManager`
...
> 
> **职责精简说明**：根据单一职责原则，ngspice 路径配置委托给 `ngspice_config`，错误解析委托给 `SpiceErrorParser`，错误恢复委托给 `SpiceErrorRecovery`，本模块专注于仿真执行核心逻辑
>
...
- [ ] **依赖的子模块**：
  - `SpiceErrorParser` - 错误解析（详见 4.2.6）
...
#### 4.2.6 `spice_error_parser.py` - SPICE错误解析器
...
- [ ] **核心功能**：
  ...
  - `apply_recovery(error, original_config, attempt_count) -> RecoveryResult` - 应用恢复策略
  - `get_user_suggestion(error: SimulationError) -> str` - 获取用户友好的恢复建议
  - `get_recovery_history() -> list[RecoveryAttempt]` - 获取恢复历史记录
  - `clear_recovery_history()` - 清空恢复历史
- [ ] **恢复策略枚举**：
