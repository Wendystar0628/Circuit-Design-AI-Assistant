...
> **⚠️ 面板职责边界（与阶段九信息面板的分工）**：
>
> - **下栏面板（本阶段）**：专注于**仿真结果的详细展示与交互操作**
>   - 波形图表查看器（支持双光标测量、缩放、导出）
>   - 高级分析结果标签页（PVT/蒙特卡洛/参数扫描等）
>   - 电路图查看器（交互式浏览）
>   - 仿真输出日志（调试用）
>   - 原始数据表格（虚拟滚动）
> - **右栏信息面板（阶段九）**：专注于**轻量级信息聚合与快速查看**
>   - 仿真状态摘要卡片（成功/失败、耗时、关键指标）
>   - 点击卡片可跳转到下栏详情
>   - 不重复实现图表查看、数据导出等功能
> - **关键区分**：下栏 = 详细数据 + 交互操作；右栏 = 摘要信息 + 快速导航
...
> - 输出闭环：每个分析模块必须有对应的可视化输出机制

#### 4.0.1 仿真域目录结构

> **⚠️ 架构重构说明（职责分离）**：
>
> - **领域层（domain/simulation/）**：专注于仿真执行、数据处理、指标提取等业务逻辑
> - **表示层（presentation/panels/simulation/）**：专注于 UI 组件、图表渲染、用户交互
> - **原 `visualization/` 目录已拆分**：数据服务保留在领域层 `data/`，UI 组件移至表示层
...
│   └── analysis_result.py           # 高级分析结果数据类（统一）
├── executor/                        # 仿真执行器
│   ├── __init__.py
...
│   ├── circuit_analyzer.py          # 电路文件分析器（含扫描功能）
│   └── simulation_task.py           # 仿真任务封装
├── analysis/                        # 高级分析模块
│   ├── __init__.py
...
│   ├── post_processor.py            # 仿真后处理（FFT/THD等）
...
├── data/                            # 仿真数据处理层（详见 4.7 节）
│   ├── __init__.py
│   ├── waveform_data_service.py     # 波形数据服务（降采样、缓存）
│   ├── downsampler.py               # LTTB 降采样算法
│   ├── resolution_pyramid.py        # 多分辨率金字塔
│   ├── data_exporter.py             # 数据导出（CSV/MATLAB/JSON）
│   └── simulation_output_reader.py  # 仿真输出日志读取
├── schematic/                       # 电路图生成子域（详见 4.8 节）
│   ├── __init__.py
│   ├── models/                      # 电路图数据模型
│   │   ├── __init__.py
│   │   ├── circuit_element.py       # 电路元件数据类
│   │   └── schematic_result.py      # 电路图生成结果数据类
│   ├── parser/                      # 网表解析器
│   │   ├── __init__.py
│   │   └── netlist_parser.py        # SPICE 网表解析器
│   ├── renderer/                    # Schemdraw 渲染适配层
│   │   ├── __init__.py
│   │   ├── schemdraw_adapter.py     # Schemdraw 绘图适配器
│   │   └── element_mapper.py        # 元件类型到 Schemdraw 元件的映射
...
│   ├── analysis_selector.py         # 分析类型选择器（详见 4.3.4 节）
│   ├── chart_selector.py            # 图表类型选择器（详见 4.3.5 节）
│   └── tuning_service.py            # 快速调参服务（可选）
└── library/                         # 元件库管理
    ├── __init__.py
    └── library_manager.py           # 子电路库管理

# domain/services/ 层（领域服务统一入口）
domain/services/
...
# presentation 层相关模块（下栏面板，详见 4.9 节）
presentation/panels/
├── simulation/                      # 仿真结果面板
│   ├── __init__.py
│   ├── simulation_panel.py          # 仿真面板主组件
...
# presentation 层对话框模块（从菜单栏设置打开）
presentation/dialogs/
├── simulation_settings_dialog.py    # 仿真设置对话框（详见 4.11.4 节）
│                                    # 包含分析类型选择和图表类型选择两个标签页
└── ...
```

#### 4.0.1.1 分析结果与输出机制对照表

> **设计原则**：每个分析模块必须有明确的输出链路，确保用户能看到分析结果
> **职责分离**：数据类在领域层定义，UI 组件在表示层实现
> **用户选择**：用户通过菜单栏「设置 → 仿真设置」打开 `SimulationSettingsDialog` 选择要执行的分析和要显示的图表

| 分析模块   | 结果数据类               | 数据服务                | UI 展示组件         | 事件名称                        |
| ---------- | ------------------------ | ----------------------- | ------------------- | ------------------------------- |
| 基础仿真   | `SimulationResult`     | `WaveformDataService` | `WaveformWidget`  | `EVENT_SIM_COMPLETE`          |
| PVT 角点   | `PVTAnalysisResult`    | `WaveformDataService` | `PVTResultTab`    | `EVENT_PVT_COMPLETE`          |
| 蒙特卡洛   | `MonteCarloResult`     | `WaveformDataService` | `MonteCarloTab`   | `EVENT_MC_COMPLETE`           |
| 参数扫描   | `SweepResult`          | `WaveformDataService` | `SweepResultTab`  | `EVENT_SWEEP_COMPLETE`        |
| 最坏情况   | `WorstCaseResult`      | `WaveformDataService` | `SensitivityTab`  | `EVENT_WC_COMPLETE`           |
| 敏感度分析 | `SensitivityResult`    | `WaveformDataService` | `SensitivityTab`  | `EVENT_SENS_COMPLETE`         |
| FFT 后处理 | `FFTResult`            | `WaveformDataService` | `WaveformWidget`  | `EVENT_FFT_COMPLETE`          |
| 拓扑识别   | `TopologyResult`       | 无                      | `SimulationPanel` | `EVENT_TOPOLOGY_DETECTED`     |
| 收敛诊断   | `ConvergenceDiagnosis` | 无                      | `SimulationPanel` | `EVENT_CONVERGENCE_DIAGNOSED` |

#### 4.0.1.2 用户选择机制

> **设计目标**：让用户完全控制要执行哪些仿真分析、要显示哪些可视化图表
> **交互方式**：通过菜单栏「设置 → 仿真设置」打开对话框进行配置

| 选择器模块 | 领域层服务 | UI 对话框 | 配置文件 | 事件名称 |
| ---------- | ---------- | ------- | -------- | -------- |
| 分析类型选择 | `AnalysisSelector` | `SimulationSettingsDialog` (分析标签页) | `analysis_selection.json` | `EVENT_ANALYSIS_SELECTION_CHANGED` |
| 图表类型选择 | `ChartSelector` | `SimulationSettingsDialog` (图表标签页) | `chart_selection.json` | `EVENT_CHART_SELECTION_CHANGED` |

**选择流程**：
1. 用户点击菜单栏「设置 → 仿真设置」打开 `SimulationSettingsDialog`
2. 在「分析类型」标签页中勾选要执行的分析类型（AC/DC/TRAN/PVT 等）
3. 在「图表显示」标签页中勾选要显示的图表类型（Bode 图/波形图/直方图等）
4. 点击「确定」保存配置，点击「应用」立即生效但不关闭对话框
5. 点击「运行仿真」按钮，`SimulationService.run_selected_analyses()` 按优先级依次执行选中的分析
6. 每个分析完成后，`ChartViewer` 根据用户选择的图表类型动态渲染对应图表
...
- [ ] **文件路径**：`presentation/dialogs/simulation_settings_dialog.py`
- [ ] **职责**：提供仿真分析类型和图表显示类型的统一配置界面
- [ ] **核心类**：`SimulationSettingsDialog(QDialog)`
- [ ] **触发方式**：菜单栏「设置 → 仿真设置」或快捷键 `Ctrl+Shift+S`
- [ ] **UI 结构**：
...
- [ ] **图表显示标签页**：
  ...
  - **布局设置组**（`QGroupBox`）：
    - 布局模式：自动 / 网格 / 自定义（单选按钮）
    - 网格列数：1-4 列（下拉框，仅网格模式可用）
  - **快捷操作**：
    - `[推荐图表]` - 根据选中的分析类型自动选择推荐图表
    - `[全选]` / `[全不选]`
- [ ] **核心功能**：
  - `load_settings()` - 从配置文件加载当前设置
  - `save_settings()` - 保存设置到配置文件
  ...
  - `update_chart_availability()` - 根据分析类型更新图表可用性
- [ ] **信号定义**：
  - `settings_changed()` - 设置变更信号
  - `settings_applied()` - 设置应用信号
- [ ] **事件发布**：
  - 点击「应用」或「确定」时发布 `EVENT_ANALYSIS_SELECTION_CHANGED`
  - 点击「应用」或「确定」时发布 `EVENT_CHART_SELECTION_CHANGED`
- [ ] **事件订阅**：
  - 分析类型勾选变更时，自动更新图表标签页的可用图表
- [ ] **国际化支持**：
  - 实现 `retranslate_ui()` 方法
  - 所有文本支持中英文切换
- [ ] **被调用方**：`main_window.py`（菜单栏触发）
