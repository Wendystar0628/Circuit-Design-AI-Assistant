...
- SPICE 仿真通过 ctypes 直接调用 ngspice 共享库（ngspice.dll / libngspice.so）执行
- 不使用 PySpice 库，避免版本兼容性问题，直接控制 ngspice C API
...
> **职责分离原则**：
> - **领域层（domain/simulation/）**：仿真执行、数据处理、测量结果解析
...
│   ├── ngspice_shared.py            # ngspice 共享库封装（ctypes）
...
│   └── post_processor.py            # 仿真后处理（群延迟、极零点等）
├── measure/                         # 声明式指标提取（基于 .MEASURE）
│   ├── measure_result.py            # 测量结果数据类
...
├── metrics_panel.py                 # 指标面板（显示 .MEASURE 结果）
...
> **⚠️ 职责精简**：本组件仅负责文件监控和事件过滤，生命周期由 `SimulationTab` 管理（而非 `SessionManager`）

- [X] **文件路径**：`domain/simulation/service/simulation_result_watcher.py`
- [X] **职责**：监控仿真结果目录的文件变化，过滤并发布结果文件创建事件
- [X] **设计原则**：
...
- [X] **生命周期管理**：
  - 由 `SimulationTab` 在初始化时启动，在销毁时停止
  - 不再由 `SessionManager` 管理，避免职责重叠
- [X] **被调用方**：`SimulationTab`

#### 4.0.6.3 事件定义扩展

- [X] **新增事件常量**（在 `shared/event_types.py` 中）：
...
- [X] **核心方法增强**：
  ...
  - `_on_session_changed(event_data)` - 处理会话变更事件
  - `_should_reload(file_path, timestamp)` - 判断是否需要重新加载
  - `_show_empty_state()` - 显示空状态占位
  - `_show_file_missing_state()` - 显示文件丢失状态
  - `refresh()` - 手动刷新，强制重新加载最新结果
- [X] **SimulationResultWatcher 生命周期管理**：
  - 在 `SimulationTab.__init__` 中创建 `SimulationResultWatcher` 实例
  - 在项目打开时调用 `_watcher.start(project_root)`
  - 在 `SimulationTab` 销毁时调用 `_watcher.stop()`

#### 4.0.6.5 版本校验机制

- [X] **`SimulationViewModel` 增强**：
  ...
  - 新增 `clear()` 方法，重置版本校验字段

> **实现说明**：已在 `presentation/panels/simulation/simulation_view_model.py` 中实现
...
> **⚠️ 核心设计决策**：仿真结果与项目绑定，而非与会话绑定

##### 4.0.7.1 设计原则

- [X] **仿真结果是项目级资源**：仿真结果存储在 `.circuit_ai/sim_results/` 目录，属于项目而非会话
- [X] **会话是对话上下文**：会话管理对话历史和工作流状态，不拥有仿真结果
- [X] **解耦带来的好处**：
  - 用户可以在不同会话中讨论同一个电路的仿真结果
  - 新会话从"干净状态"开始，避免旧数据干扰
  - 用户可以随时重新仿真获取最新结果

##### 4.0.7.2 新会话启动时的仿真面板行为

- [X] **默认行为：不自动加载历史仿真结果**
  - 新会话启动时，仿真面板显示"暂无仿真结果"占位状态
  - 用户可以通过"加载历史结果"按钮主动加载
  - 用户可以通过"运行仿真"按钮获取新结果
- [X] **设计理由**：
  - 避免用户误以为旧结果是当前电路的最新状态
  - 鼓励用户在修改电路后重新仿真
  - 保持会话的独立性和可预测性

##### 4.0.7.3 会话切换时的仿真面板行为

- [X] **切换到已有会话**：
  - 如果该会话的 `GraphState.sim_result_path` 有值且文件存在，加载显示
  - 如果文件不存在（悬空指针），显示"结果文件已丢失"提示
- [X] **切换到新会话**：
  - 清空仿真面板，显示"暂无仿真结果"占位状态
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  [项目打开事件: EVENT_STATE_PROJECT_OPENED]                             │
│       │                                                                 │
│       ▼                                                                 │
│  SessionManager 创建/恢复会话                                           │
│       │                                                                 │
│       ├──► 新会话：GraphState.sim_result_path = ""                      │
│       │                                                                 │
│       └──► 恢复会话：GraphState.sim_result_path = "sim_results/xxx.json"│
│                                                                         │
│       │                                                                 │
│       ▼                                                                 │
│  SimulationTab._on_session_changed()                                    │
│       │                                                                 │
│       ├──► sim_result_path 为空？                                       │
│       │         │                                                       │
│       │         ├── 是 → 显示"暂无仿真结果"占位状态                     │
...
│       │                                                                 │
│       ▼                                                                 │
│  启动 SimulationResultWatcher（监控新结果文件）                         │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```python
# 会话变更事件（由 SessionManager 发布）
# 携带数据：
#   - session_id: str - 新会话 ID
#   - sim_result_path: str - 仿真结果路径（可能为空）
EVENT_SESSION_CHANGED = "session_changed"
```

##### 4.0.7.6 SimulationTab 会话响应增强

- [X] **新增事件订阅**：
  - 订阅 `EVENT_SESSION_CHANGED`，在 `_on_session_changed` 中处理
- [X] **核心方法**：
  - `_on_session_changed(event_data)` - 处理会话变更
  - `_show_empty_state()` - 显示空状态占位
  - `_show_file_missing_state()` - 显示文件丢失状态
  - `_load_from_path(sim_result_path)` - 从路径加载结果

---

#### 4.0.8 仿真结果时间戳显示机制

> **⚠️ 核心需求**：用户需要清楚知道当前显示的仿真结果是什么时候生成的，避免误将旧结果当作最新状态

##### 4.0.8.1 设计原则

- [X] **始终显示时间戳**：仿真面板在有结果时，必须显示该结果的生成时间
- [X] **时间戳来源**：从 `SimulationResult.timestamp` 字段获取（ISO 格式）
- [X] **显示位置**：在仿真面板左栏顶部，指标网格上方
- [X] **显示格式**：本地化的日期时间格式，如 "2026-01-06 14:30:22"

##### 4.0.8.2 SVG 图标资源

> **设计原则**：所有图标使用 SVG 格式，统一存放在 `resources/icons/simulation/` 目录

- [X] **文件路径**：`resources/icons/simulation/`
- [X] **图标列表**：
  - `clock.svg` - 时钟图标（用于时间戳显示）
  - `chart-empty.svg` - 空图表占位图标（用于空状态）
  - `file-missing.svg` - 文件丢失图标（用于文件丢失状态）
- [X] **图标规格**：
  - 尺寸：16x16 或 24x24
  - 颜色：使用 `currentColor` 以支持主题切换
  - 风格：线条风格，与项目现有图标保持一致

##### 4.0.8.3 顶部信息栏布局（紧凑单行设计）

> **设计原则**：将仿真时间、总体分数、刷新按钮和历史按钮整合到同一行，提高空间利用率

- [X] **更新 `MetricsSummaryPanel`**：
  - 顶部使用单行 `_header_bar: QFrame` 替代原有的时间戳栏和底部操作栏
  - 布局结构：`[时钟图标][时间戳文本] | [Overall Score 标签][分数值][进度条] | [刷新按钮][历史按钮]`
  - 新增 `set_result_timestamp(timestamp: str)` 方法
  - 新增 `clear_result_timestamp()` 方法
  - 新增 `_format_timestamp(iso_str) -> str` 方法（ISO 格式转本地化显示）
  - 新增 `set_overall_score(score: float)` 方法（更新分数显示和进度条）
- [X] **UI 设计**：
  - 顶部信息栏高度：36px
  - 背景色：纯白（`#ffffff`）
  - 时间戳区域：时钟图标 + 文本，左对齐
  ...
  - 各区域之间使用弹性空间分隔
- [X] **更新 `SimulationTab`**：
  - 在 `load_result()` 方法中调用 `_metrics_summary_panel.set_result_timestamp(result.timestamp)`
  - 在 `clear()` 方法中调用 `_metrics_summary_panel.clear_result_timestamp()`
  - 在 `_show_empty_state()` 中隐藏顶部信息栏
  - 在 `_hide_empty_state()` 中显示顶部信息栏

##### 4.0.8.4 空状态 UI 增强

> **设计目标**：空状态下提供"加载历史结果"按钮，让用户可以主动加载历史仿真结果

- [X] **空状态 UI 元素**（更新 `SimulationTab._empty_widget`）：
  - 图标：`resources/icons/simulation/chart-empty.svg`
  - 主文本："暂无仿真结果"
  - 副文本："运行仿真后，结果将显示在此处"
  - 新增按钮："加载历史结果" - 点击后加载最新的仿真结果文件
- [X] **核心方法**（更新 `SimulationTab`）：
  - `_on_load_history_clicked()` - 处理"加载历史结果"按钮点击
  - 复用 `_load_project_simulation_result()` 方法加载最新结果
- [X] **按钮样式**：
  - 使用 `QPushButton`，样式与刷新按钮一致
  - 居中显示在副文本下方

##### 4.0.8.5 文件丢失状态 UI

> **设计目标**：当仿真结果文件丢失时，显示明确的提示和操作按钮

- [X] **文件丢失状态 UI 元素**（更新 `SimulationTab._show_file_missing_state()`）：
  - 图标：`resources/icons/simulation/file-missing.svg`
  - 主文本："仿真结果文件已丢失"
  - 副文本："请重新运行仿真或点击刷新按钮"
...
>
> **⚠️ 关键约束**：此模块必须在应用启动时最先执行，确保 ngspice DLL 路径正确配置

- [X] **文件路径**：`infrastructure/utils/ngspice_config.py`（已实现）
- [X] **职责**：管理 ngspice 共享库和模型库的路径配置，处理跨平台差异
- [X] **核心功能**：
  - `configure_ngspice() -> bool` - 配置 ngspice 路径
  - `get_ngspice_dll_path() -> Path` - 获取 ngspice 共享库路径（ngspice.dll / libngspice.so）
...
- [X] **目录结构**（以 Windows 为例）：
  ...
  └── macos/
      └── .gitkeep
  ```
- [X] **版本要求**：ngspice 40 或更高版本（已验证 ngspice 42 兼容）
- [X] **许可证**：ngspice 使用 BSD-3-Clause 许可证，允许二进制分发

#### 4.2.4.1 `ngspice_shared.py` - ngspice 共享库封装（已完成）

> **设计原则**：使用 ctypes 直接调用 ngspice C API，不依赖 PySpice，避免版本兼容性问题
>
> **⚠️ 为什么不使用 PySpice**：
> - PySpice 的 CFFI 定义与 ngspice 42 存在兼容性问题（`duplicate declaration of struct ngcomplex`）
> - PySpice 的 SpiceParser 解析网表时节点检测不完整
> - 直接使用 ctypes 调用 ngspice 更可控，约 200-300 行代码即可实现核心功能

- [X] **文件路径**：`domain/simulation/executor/ngspice_shared.py`
- [X] **职责**：封装 ngspice 共享库的 C API，提供 Python 友好的接口
- [X] **核心类**：`NgSpiceWrapper`
- [X] **ngspice C API 封装**：
  - `ngSpice_Init()` - 初始化 ngspice，设置回调函数
  - `ngSpice_Circ()` - 加载网表（字符串数组形式）
  - `ngSpice_Command()` - 执行 ngspice 命令（如 "run"、"ac dec 10 1 1meg"）
  - `ngSpice_CurPlot()` - 获取当前 plot 名称
  - `ngSpice_AllVecs()` - 获取当前 plot 的所有向量名称
  - `ngGet_Vec_Info()` - 获取向量数据（频率、电压、电流等）
- [X] **回调函数**：
  - `SendChar` - 接收 ngspice 的文本输出（stdout/stderr）
  - `SendStat` - 接收仿真状态更新
  - `ControlledExit` - 处理 ngspice 退出请求
  - `SendData` - 接收仿真数据（可选，用于实时数据流）
  - `SendInitData` - 接收初始化数据（可选）
- [X] **核心方法**：
  - `__init__(dll_path: Path)` - 加载 ngspice DLL
  - `load_netlist(netlist_lines: List[str]) -> bool` - 加载网表
  - `load_netlist_file(file_path: Path) -> bool` - 从文件加载网表
  - `run() -> bool` - 执行仿真
  - `execute_command(command: str) -> bool` - 执行任意 ngspice 命令
  - `get_current_plot() -> str` - 获取当前 plot 名称
  - `get_all_vectors() -> List[str]` - 获取所有向量名称
  - `get_vector_data(vec_name: str) -> Optional[np.ndarray]` - 获取向量数据
  - `get_complex_vector_data(vec_name: str) -> Optional[np.ndarray]` - 获取复数向量数据
  - `get_stdout() -> str` - 获取 ngspice 输出日志
  - `halt() -> bool` - 停止当前仿真
  - `reset() -> bool` - 重置 ngspice 状态
- [X] **数据结构**（对应 ngspice C 结构体）：
  ```python
  @dataclass
  class VectorInfo:
      """向量信息"""
...
- [X] **线程安全**：
  ...
  - 使用 `threading.Lock` 保护所有 ngspice 调用
- [X] **错误处理**：
  - DLL 加载失败：抛出 `NgSpiceLoadError`
  - 命令执行失败：返回 False 并记录错误日志
  - 向量不存在：返回 None
- [X] **被调用方**：`spice_executor.py`

#### 4.2.5 `spice_executor.py` - SPICE仿真执行器（已完成）

> **初始化顺序**：无需显式初始化，作为工具类按需实例化，依赖 `ngspice_config`（路径配置）、`ngspice_shared`（共享库封装）
>
> **⚠️ 执行模式说明**：通过 ctypes 直接调用 ngspice 共享库，ngspice 在**同一进程内**执行，不需要启动独立子进程。这种模式性能更高，且完全控制与 ngspice 的交互。
>
> **职责精简说明**：根据单一职责原则，ngspice 路径配置委托给 `ngspice_config`，共享库调用委托给 `ngspice_shared`，错误解析作为执行器内部职责（`_parse_ngspice_output` 方法），本模块专注于仿真执行核心逻辑
>
> **⚠️ 路径解析策略**：采用工作目录切换方案，利用 ngspice 原生的相对路径解析能力，无需生成临时文件

- [X] **文件路径**：`domain/simulation/executor/spice_executor.py`
- [X] **职责**：实现 `SimulationExecutor` 接口，通过 `NgSpiceWrapper` 执行 SPICE 电路仿真
...
- [X] **仿真执行流程**：
  ...
  4. 加载电路文件内容
  5. 动态注入分析命令到网表（根据 analysis_config 生成 .ac/.dc/.tran 等命令）
  6. 通过 `NgSpiceWrapper.load_netlist()` 加载网表
  7. 通过 `NgSpiceWrapper.run()` 执行仿真
  8. 通过 `NgSpiceWrapper.get_vector_data()` 提取仿真数据
  9. 如果仿真失败，解析 ngspice 输出并返回结构化错误
  10. **恢复原工作目录**（使用 try-finally 确保恢复）
  11. 返回 SimulationResult
- [X] **分析命令注入**：
  - 读取原始网表内容
  - 根据 `analysis_config` 生成分析命令（如 `.ac dec 20 1 1g`）
  - 在 `.end` 之前插入分析命令
  - 将修改后的网表传递给 `NgSpiceWrapper`
- [X] **依赖**：
  - `NgSpiceWrapper` - ngspice 共享库封装
  - `infrastructure.utils.ngspice_config` - ngspice 路径配置
...
> **设计原则**：错误解析是仿真执行的内部职责，与执行逻辑紧密耦合，不单独分离为独立模块。错误信息来自 ngspice 的 stdout/stderr 回调，通过 `NgSpiceWrapper` 收集。

- [X] **核心方法**：`_parse_ngspice_output(stdout: str, file_path: str) -> Optional[SimulationError]`
- [X] **职责**：解析 ngspice 输出，提取错误信息并转换为结构化的 `SimulationError` 对象
- [X] **增强功能**：
  - `_extract_line_number(error_msg)` - 从错误消息中提取行号
  - `_extract_missing_models(error_msg)` - 从错误消息中提取缺失模型名称
  - `_extract_floating_nodes(error_msg)` - 从错误消息中提取浮空节点名称
- [X] **错误分类规则**：
  - 语法错误：匹配 "syntax"、"parse"、"Error:" 关键词
  - 模型缺失：匹配 "model"、"not found"、"unknown" 关键词
  - 节点浮空：匹配 "floating"、"no dc path" 关键词
  - 收敛失败：匹配 "convergence"、"no convergence"、"singular matrix" 关键词
  - 超时：匹配 "timeout" 关键词
- [X] **返回值**：`SimulationError` 对象，包含错误码、类型、严重级别、恢复建议等
- [X] **设计决策说明**：
  - 不创建独立的 `SpiceErrorParser` 模块
  - 原因：错误解析与仿真执行紧密耦合，内聚处理更合理
...
- [ ] **错误解析实现：**
  ...
  - `spice_executor._parse_ngspice_output()` 方法负责解析 ngspice 输出并转换为结构化的 `SimulationError`
  - 每个错误类型包含：
...
- [X] **事件发布**：
  ...
  - `EVENT_PVT_COMPLETE` - PVT 分析完成
  - `EVENT_MC_COMPLETE` - 蒙特卡洛分析完成
  - `EVENT_SWEEP_COMPLETE` - 参数扫描完成
  - `EVENT_WORST_CASE_COMPLETE` - 最坏情况分析完成
  - `EVENT_SENSITIVITY_COMPLETE` - 敏感度分析完成
- [X] **依赖**：
  - `ExecutorRegistry` - 获取执行器
  - `CircuitAnalyzer` - 扫描可仿真文件和检测主电路
  - `AnalysisSelector` - 获取用户选中的分析类型
  - `SimulationConfigService` - 获取分析配置
  - `PVTAnalyzer` - PVT 角点分析
  - `MonteCarloAnalyzer` - 蒙特卡洛分析
  - `ParametricSweep` - 参数扫描
  - `WorstCaseAnalyzer` - 最坏情况分析
  - `SensitivityAnalyzer` - 敏感度分析
- [X] **被调用方**：`simulation_worker.py`、`main_window.py`、`tool_executor.py`

##### 4.3.1.1 高级分析执行链路

> **⚠️ 关键设计**：高级分析（PVT/蒙特卡洛/参数扫描/最坏情况/敏感度）需要在 `run_selected_analyses` 中显式调用对应的分析模块

- [X] **高级分析执行流程**：
  1. `run_selected_analyses` 获取用户选中的分析类型列表
  2. 对于基础分析（AC/DC/TRAN/NOISE/OP），调用 `run_simulation` 执行
  3. 对于高级分析，调用对应的分析模块：
     - PVT → `PVTAnalyzer.run_pvt_corners()`
     - MONTE_CARLO → `MonteCarloAnalyzer.run_monte_carlo()`
     - PARAMETRIC → `ParametricSweep.run_sweep()`
     - WORST_CASE → `WorstCaseAnalyzer.run_worst_case()`
     - SENSITIVITY → `SensitivityAnalyzer.run_sensitivity()`
  4. 每个高级分析完成后发布对应的完成事件
  5. 高级分析结果保存到 `.circuit_ai/sim_results/` 目录

- [X] **高级分析事件发布**：
  - 高级分析模块内部发布进度事件（如 `EVENT_PVT_CORNER_COMPLETE`）
  - `run_selected_analyses` 在高级分析完成后发布完成事件（如 `EVENT_PVT_COMPLETE`）
  - 完成事件携带完整的分析结果对象

- [X] **高级分析结果存储**：
  - 高级分析结果使用独立的文件格式：`{uuid}_{analysis_type}.json`
  - 结果文件包含完整的分析数据，供 UI 展示使用
  - 结果路径记录在 `EVENT_*_COMPLETE` 事件数据中

- [X] **UI 响应链路**：
  1. `AdvancedAnalysisTabGroup` 订阅高级分析完成事件
  2. 收到事件后自动切换到对应的结果标签页
  3. 各结果标签页（如 `PVTResultTab`）从事件数据中获取结果并显示
...
#### 4.3.6 `tuning_service.py` - 快速调参服务

> **文件位置**：`domain/simulation/service/tuning_service.py`
>
> **设计说明**：参数提取功能已由 `parameter_extractor.py` 实现，本服务专注于调参仿真的执行流程

- [X] **文件路径**：`domain/simulation/service/tuning_service.py`
- [X] **职责**：提供调参仿真功能，支持参数修改、仿真执行和文件恢复
- [X] **核心功能**：
  - `apply_parameter_changes(file_path, changes) -> TuningApplyResult` - 应用参数修改到电路文件
  - `run_tuning_simulation(file_path, changes, project_root, analysis_config) -> SimulationResult` - 执行调参仿真
  - `restore_original(file_path) -> bool` - 恢复原始文件
  - `get_backup_path(file_path) -> Path` - 获取备份文件路径
- [X] **数据类**：
  - `TuningApplyResult` - 参数应用结果
    - `success: bool` - 是否成功
    - `modified_lines: List[int]` - 修改的行号列表
...
- [X] **参数修改规则**：
  ...
  - 保留原有单位前缀格式
- [X] **事件发布**：
  - `EVENT_TUNING_APPLIED` - 参数已应用
  - `EVENT_TUNING_RESTORED` - 文件已恢复
- [X] **被调用方**：`TuningPanel`（UI）、`SimulationService`
- [X] **模块级单例**：`tuning_service`
...
#### 4.4.6 `post_processor.py` - 仿真数据后处理

> **初始化顺序**：无需显式初始化，作为工具类按需实例化
>
> **设计说明**：FFT/THD/SNDR/SFDR/ENOB 等频谱分析功能已在 `distortion_metrics.py` 中完整实现，
> 本模块专注于 AC 分析数据的高级处理（极零点、群延迟）和通用数据处理（插值、平滑）

- [x] **文件路径**：`domain/simulation/analysis/post_processor.py`
- [x] **职责**：对仿真数据进行后处理，提取传递函数特征，提供数据处理工具
- [x] **核心功能**：
  - `find_poles_zeros(ac_data, order) -> PoleZeroResult` - 从 AC 分析数据提取极点和零点
  - `compute_group_delay(ac_data) -> GroupDelayResult` - 计算群延迟
  - `compute_phase_margin(ac_data) -> PhaseMarginResult` - 计算相位裕度和增益裕度
  - `interpolate_data(x, y, new_x, method) -> np.ndarray` - 数据插值
  - `smooth_data(data, window_size, method) -> np.ndarray` - 数据平滑
  - `resample_uniform(time, signal, num_points) -> Tuple[np.ndarray, np.ndarray]` - 均匀重采样
- [x] **极零点提取**：
  - 使用向量拟合（Vector Fitting）或最小二乘法从 AC 数据拟合传递函数
  - 提取主极点和零点位置（复数形式）
  - 计算品质因数 Q 和自然频率 ωn
  - 返回 `PoleZeroResult` 数据类
- [x] **群延迟计算**：
  - 群延迟定义：τg = -dφ/dω
  - 使用数值微分计算相位对频率的导数
  - 支持相位展开（unwrap）处理
  - 返回 `GroupDelayResult` 数据类
- [x] **稳定性分析**：
  - 计算相位裕度（Phase Margin）
  - 计算增益裕度（Gain Margin）
  - 识别单位增益频率（Unity Gain Frequency）
  - 返回 `PhaseMarginResult` 数据类
- [x] **数据处理工具**：
  - 插值方法：线性、三次样条、Akima
  - 平滑方法：移动平均、Savitzky-Golay
  - 均匀重采样：将非均匀采样数据转换为均匀采样
- [x] **结果数据类**（定义在本模块内）：
  - `PoleZeroResult` - 极零点结果
  - `GroupDelayResult` - 群延迟结果
  - `PhaseMarginResult` - 相位/增益裕度结果
- [x] **被调用方**：`amplifier_metrics.py`（稳定性指标）、`chart_viewer.py`（群延迟图）、`waveform_data_service.py`（数据处理）

### 4.5 声明式指标提取模块 (`domain/simulation/measure/`)

> **⚠️ 架构重构说明**：
>
> 原设计采用"拓扑识别 + 静态映射 + 代码计算"的方案，存在以下问题：
> - 拓扑识别置信度低（0.4-0.7），对非标准电路识别能力差
> - 静态映射无法适应同一拓扑下的不同变体
> - 从原始波形计算指标的代码复杂且容易出错
> - 维护成本高，bug 难以修复
>
> **新方案：基于 .MEASURE 语句的声明式指标提取**
>
> 核心思路：
> 1. 将指标定义从代码迁移到网表（使用 ngspice 原生 .MEASURE 语句）
> 2. LLM 根据用户需求和电路分析生成 .MEASURE 语句
> 3. 仿真器直接计算指标，系统只需解析 .MEASURE 结果
> 4. 删除拓扑识别和静态映射，大幅简化架构
>
> **优势**：
> - 精度高：仿真器原生计算，避免后处理误差
> - 灵活性强：LLM 可为任意电路生成合适的测量语句
> - 代码简洁：删除数千行指标计算代码
> - 可维护性好：指标定义在网表中，不在代码中

> **目录结构**：
>
> ```
> domain/simulation/measure/
> ├── __init__.py
...
#### 4.5.1 `measure_result.py` - 测量结果数据类

- [ ] **文件路径**：`domain/simulation/measure/measure_result.py`
- [ ] **职责**：定义 .MEASURE 结果的标准化数据结构
- [ ] **数据类定义**：
  - `MeasureResult` - 单个测量结果
    - `name: str` - 测量名称（.MEASURE 语句中定义的名称）
    - `value: Optional[float]` - 测量值
    - `unit: str` - 单位（从测量类型推断或用户指定）
    - `analysis_type: str` - 分析类型（ac/dc/tran/noise）
    - `success: bool` - 测量是否成功
    - `error_message: Optional[str]` - 失败时的错误信息
    - `raw_output: Optional[str]` - ngspice 原始输出
  - `MeasureResultCollection` - 测量结果集合
    - `results: Dict[str, MeasureResult]` - 测量名称到结果的映射
    - `analysis_type: str` - 分析类型
    - `timestamp: str` - 测量时间
- [ ] **核心方法**：
  - `to_dict()` - 序列化为字典
  - `from_dict(data)` - 从字典反序列化
  - `get_value(name, default)` - 获取指定测量值
  - `is_all_successful()` - 检查所有测量是否成功
- [ ] **被调用方**：`measure_parser.py`、`simulation_result.py`、`metrics_panel.py`

#### 4.5.2 `measure_parser.py` - 测量结果解析器

- [ ] **文件路径**：`domain/simulation/measure/measure_parser.py`
- [ ] **职责**：解析 ngspice 输出中的 .MEASURE 结果
- [ ] **核心功能**：
  - `parse_measure_output(ngspice_output: str) -> MeasureResultCollection` - 解析 ngspice 输出
  - `extract_measure_value(line: str) -> Tuple[str, float]` - 从单行提取测量名和值
  - `detect_measure_errors(output: str) -> List[str]` - 检测测量错误
- [ ] **ngspice .MEASURE 输出格式**：
  ```
  gain_db                 =  2.000000e+01
  f_3db                   =  1.591549e+06
  phase_margin            =  6.500000e+01
  ```
- [ ] **错误处理**：
  - 测量失败（如条件未满足）：返回 `MeasureResult` 并设置 `success=False`
  - 解析错误：记录日志并跳过该测量
- [ ] **被调用方**：`spice_executor.py`

#### 4.5.3 `measure_injector.py` - 测量语句注入器

- [ ] **文件路径**：`domain/simulation/measure/measure_injector.py`
- [ ] **职责**：将 .MEASURE 语句注入到网表中
- [ ] **核心功能**：
  - `inject_measures(netlist: str, measures: List[str]) -> str` - 注入测量语句
  - `remove_existing_measures(netlist: str) -> str` - 移除现有测量语句
  - `validate_measure_syntax(measure: str) -> bool` - 校验测量语句语法
- [ ] **注入位置**：在 `.end` 语句之前插入
- [ ] **被调用方**：`spice_executor.py`
...
- [x] **文件路径**：`domain/simulation/helpers/convergence_helper.py`
- [x] **职责**：辅助解决仿真收敛问题，诊断问题原因，生成修复建议，并将诊断结果展示给用户
- [x] **核心功能**：
  ...
  - `get_convergence_param_suggestions(issue_type) -> Dict` - 获取收敛参数调整建议
...
- [x] **收敛问题诊断**：
  ...
  - 浮空节点：检测无 DC 通路到地的节点
  - 模型问题：检测缺失或无效的模型定义
- [x] **修复建议生成**（不执行自动修复，由 LLM 或用户决定）：
  - 浮空节点：建议添加高阻接地电阻（如 1GΩ）
  - DC 不收敛：建议放宽 gmin、reltol、abstol 参数
  - 瞬态不收敛：建议减小最大时间步长、添加 .ic 初始条件
  - 模型问题：建议检查模型文件路径或使用内置模型
- [x] **初始条件建议**：
  - 分析电路拓扑，识别需要初始条件的节点
  - 根据电源电压和电路结构估算合理的初始值
  - 生成 .ic 语句建议
- [x] **网表验证**：
  - 检查是否有地节点（0/gnd/ground/vss）
  - 检查是否有电源（V 或 I 源）
  - 检查 .include 文件引用
- [x] **结果数据结构**：返回 `ConvergenceDiagnosis`（定义见 4.1.4 节）
- [x] **事件发布**：
...
- [x] **被调用方**：`spice_executor.py`（仿真失败时）、`DiagnosisPanel`（UI 显示）
...
- [x] **文件路径**：`domain/simulation/data/resolution_pyramid.py`
- [x] **职责**：管理波形数据的多分辨率金字塔，支持快速缩放
- [x] **设计原则**：
  - 金字塔数据按需构建，缓存在内存中
  - 不单独持久化（原始数据已在仿真结果文件中）
  - 作为 `WaveformDataService` 的内部实现细节
- [x] **数据类定义**：
  - `PyramidLevel` - 单个分辨率层级
    - `target_points: int` - 目标点数
    - `x_data: np.ndarray` - X 轴数据
    - `y_data: np.ndarray` - Y 轴数据
    - `actual_points: int` - 实际点数（可能小于目标点数）
  - `PyramidData` - 金字塔数据容器
    - `original_points: int` - 原始数据点数
    - `levels: List[PyramidLevel]` - 各层级数据（按点数升序排列）
    - `x_range: Tuple[float, float]` - X 轴范围
    - `y_range: Tuple[float, float]` - Y 轴范围
- [x] **核心功能**：
  - `build_pyramid(x: np.ndarray, y: np.ndarray, levels: List[int] = None) -> PyramidData` - 构建金字塔
  - `select_optimal_level(pyramid: PyramidData, required_points: int) -> int` - 选择最优层级索引
  - `get_level_data(pyramid: PyramidData, level_index: int) -> Tuple[np.ndarray, np.ndarray]` - 获取指定层级数据
- [x] **默认层级**：`DEFAULT_PYRAMID_LEVELS = [500, 2000, 10000, 50000]` 点
- [x] **层级选择策略**：选择第一个 `target_points >= required_points` 的层级，若无则返回最高分辨率层级
- [x] **被调用方**：`WaveformDataService`

#### 4.7.4 `data_exporter.py` - 数据导出服务

- [x] **文件路径**：`domain/simulation/data/data_exporter.py`
- [x] **职责**：将仿真数据导出为多种标准格式
- [x] **核心功能**：
  - `export(data, format, path, signals)` - 统一导出接口
  - `export_with_result(data, format, path, signals)` - 导出并返回详细结果
  - `get_supported_formats() -> List[str]` - 获取支持的导出格式列表
  - 支持格式：CSV、JSON、MATLAB (.mat)、NumPy (.npy/.npz)
- [x] **被调用方**：`presentation/panels/simulation/chart_viewer.py`

#### 4.7.5 `simulation_output_reader.py` - 仿真输出日志读取

- [x] **文件路径**：`domain/simulation/data/simulation_output_reader.py`
- [x] **职责**：读取和解析仿真引擎的输出日志
- [x] **核心功能**：
...
- [x] **被调用方**：`presentation/panels/simulation/output_log_viewer.py`
...
- [x] **文件路径**：`presentation/dialogs/simulation_config/simulation_config_dialog.py`
- [x] **职责**：协调各配置标签页，管理对话框整体布局和按钮行为
- [x] **核心类**：`SimulationConfigDialog(QDialog)`
- [x] **UI 布局**：
  - 顶部：标题栏
...
- [x] **被调用方**：`main_window.py`（菜单项「设置 → 仿真参数配置」触发）
- [x] **菜单集成**：
  - 菜单项：`menu.settings.simulation_config`
  - 快捷键：`Ctrl+Alt+,`
  - 回调方法：`ActionHandlers.on_simulation_config()`

#### 4.10.4.3 ~ 4.10.4.7 配置标签页（已内联实现）

> **实现说明**：以下5个配置标签页已在 `simulation_config_dialog.py` 中通过 `_create_*_tab()` 方法内联实现，不再拆分为独立文件。
>
> **设计决策理由**：
> 1. 每个标签页UI逻辑简单（约50-80行），拆分后反而增加接口复杂度
> 2. ViewModel已提供良好的数据绑定机制，标签页只是UI壳
> 3. 信号连接在主对话框中统一管理，避免跨文件传递
> 4. 单文件便于维护和调试

##### 4.10.4.3 AC 分析配置（`_create_ac_tab()`）

- [x] **实现位置**：`simulation_config_dialog.py` 第 220-280 行
- [x] **UI 组件**：
...
##### 4.10.4.4 DC 分析配置（`_create_dc_tab()`）

- [x] **实现位置**：`simulation_config_dialog.py` 第 282-340 行
- [x] **UI 组件**：
  - 扫描源名称输入框（QLineEdit）
  - 起始值/终止值/步进值输入框（QDoubleSpinBox）
  - 参数说明标签

##### 4.10.4.5 瞬态分析配置（`_create_transient_tab()`）

- [x] **实现位置**：`simulation_config_dialog.py` 第 342-420 行
- [x] **UI 组件**：
  - 时间步长/终止时间/起始时间/最大步长输入框（QDoubleSpinBox）
  - 使用初始条件复选框（QCheckBox）
  - 参数说明标签

##### 4.10.4.6 噪声分析配置（`_create_noise_tab()`）

- [x] **实现位置**：`simulation_config_dialog.py` 第 422-480 行
- [x] **UI 组件**：
  - 输出节点/输入源输入框（QLineEdit）
  - 起始频率/终止频率输入框（QDoubleSpinBox）
  - 参数说明标签

##### 4.10.4.7 收敛参数配置（`_create_convergence_tab()`）

- [x] **实现位置**：`simulation_config_dialog.py` 第 482-580 行
- [x] **UI 组件**：
  - gmin/abstol/reltol/vntol 输入框（QDoubleSpinBox）
  - itl1/itl4 输入框（QSpinBox）
  - 超时时间/仿真温度输入框
  - 参数说明标签
...
> │   ├── simulation_tab.py           # 仿真结果标签页（含 StatusIndicator 状态指示器）
...
- [x] **DisplayMetric 数据结构**（UI 友好格式）：
  ...
  - `value` - 格式化后的数值字符串
  - `unit` - 单位
  - `target` - 目标值描述（无目标时为空字符串）
  - `is_met` - 是否达标（无目标时为 `None`）
...
- [x] **无目标模式支持**：
  - `has_goals: bool` - 属性，指示当前是否有设计目标
  - `_calculate_overall_score()` 方法在无目标时返回 -1.0
  - `_create_simple_display_metric()` 方法在无目标时正常创建指标，`is_met=None`
  - UI 层根据 `has_goals` 属性决定是否显示达标状态和综合评分
...
##### 4.11.1.0.1 无目标模式指标显示规范

> **⚠️ 核心设计原则**：指标显示与设计目标解耦，即使没有设置设计目标，仿真结果中的指标值也应正常显示

**问题场景**：
- 用户完成仿真后，波形和原始数据正常显示
- 但指标面板全部显示 N/A，因为没有设置设计目标
- 用户期望：即使没有目标，也能看到仿真提取的指标测量值

**设计决策**：
- 指标显示分为两种模式：**有目标模式**和**无目标模式**
- 无目标模式下，指标卡片仅显示测量值，不显示达标状态
- 综合评分在无目标模式下显示为"N/A"或基于有效指标比例计算

**无目标模式行为规范**：

1. **指标卡片（MetricCard）**：
   - 显示指标名称和测量值
   - 隐藏达标状态图标（`is_met` 为 `None` 时）
   - 隐藏目标值描述（`target` 为空时）
   - 趋势指示正常显示（若有历史数据）
...
3. **SimulationViewModel**：
   - `_load_metrics_from_dict()` 方法在无目标时正常创建 `DisplayMetric`
   - `_calculate_overall_score()` 方法在无目标时返回 -1 或特殊值表示"无评分"
   - 新增 `has_goals` 属性，指示当前是否有设计目标
   - `_detect_output_signal()` 方法自动检测输出信号名称

4. **指标提取流程**：
   - `MetricsExtractor.extract_all_metrics()` 始终提取可计算的指标
   - 指标的 `target` 和 `is_met` 字段在无目标时为 `None`/空
   - 指标的 `value` 和 `raw_value` 字段始终有值（若计算成功）

**数据流示意**：
```
仿真完成 → SimulationViewModel.load_result()
         → _detect_output_signal() 自动检测输出信号
         → MetricsExtractor.extract_all_metrics(output_signal=detected)
         → 返回 Dict[str, MetricResult]（无目标信息）
         → SimulationViewModel.format_metric()
         → DisplayMetric（target="", is_met=None）
         → MetricCard.set_from_display_metric()
         → 显示测量值，隐藏达标状态
```

**输出信号自动检测规则**：
- 优先匹配 `out_mag`、`V(out)_mag`、`vout_mag` 等包含 "out" 和 "mag" 的信号
- 其次匹配 `V(out)`、`vout`、`out` 等包含 "out" 的信号
- 若无匹配，使用第一个非电流信号（不含 `#branch`）
- 若仍无匹配，返回 `None`，跳过需要输出信号的指标提取

##### 4.11.1.1 `presentation/panels/simulation/metric_card.py` - 指标卡片组件

- [x] **文件路径**：`presentation/panels/simulation/metric_card.py`
- [x] **职责**：专注于单个性能指标的卡片式渲染，支持有目标和无目标两种显示模式
- [x] **核心类**：`MetricCard(QFrame)`
- [x] **核心功能**：
  - `set_metric(name, value, unit, target, is_met)` - 设置指标数据
  - `set_from_display_metric(metric)` - 从 DisplayMetric 对象设置数据
  - `set_highlight(enabled)` - 设置高亮状态
  - `update_status(is_met)` - 更新达标状态
- [x] **无目标模式显示**：
  - 当 `is_met` 为 `None` 时，隐藏达标状态图标
  - 当 `target` 为空时，隐藏目标值标签
  - 指标名称和测量值始终显示
  - 趋势指示正常显示（若有历史数据）
- [x] **视觉设计**：
  - 卡片背景：略深于面板背景 `#f0f4f8`
  ...
  - 指标名称：小字灰色 `#666666`
  - 指标值：大字主色 `#333333`
  - 目标值：小字显示目标范围 `#888888`（无目标时隐藏）
  - 达标图标：右上角，绿色勾选 `#4caf50` / 红色叉号 `#f44336`（无目标时隐藏）
- [x] **被调用方**：`metrics_panel.py`

##### 4.11.1.2 `presentation/panels/simulation/metrics_panel.py` - 指标显示面板

- [x] **文件路径**：`presentation/panels/simulation/metrics_panel.py`
- [x] **职责**：专注于指标网格布局和综合评分显示，支持有目标和无目标两种模式
- [x] **核心类**：`MetricsPanel(QWidget)`
- [x] **核心功能**：
  - `update_metrics(metrics_list)` - 更新所有指标卡片
  - `set_overall_score(score)` - 设置综合评分（-1 表示无评分）
  - `clear()` - 清空所有指标
- [x] **无目标模式显示**：
  - 指标卡片正常显示测量值
  - 综合评分显示为"N/A"（当 score < 0 时）
  - 评分进度条显示为灰色或隐藏
- [x] **布局**：
...
- [X] **文件路径**：`presentation/panels/simulation/tuning_panel.py`
- [X] **职责**：提供可视化的参数调整界面，支持滑块调参和实时仿真更新
- [X] **核心类**：`TuningPanel(QWidget)`、`ParameterSliderWidget(QFrame)`、`ParameterState(dataclass)`
- [X] **核心功能**：
  - `load_parameters(param_list)` - 加载可调参数列表
  - `load_from_file(file_path)` - 从电路文件加载参数
  - `get_modified_parameters()` - 获取已修改的参数
  - `get_all_parameters()` - 获取所有参数当前值
  - `set_auto_simulation(enabled)` - 设置自动仿真模式
  - `clear()` - 清空所有参数
  - `retranslate_ui()` - 国际化支持
- [X] **UI 组件**：
  - 参数列表区：显示所有可调参数，每个参数一行
  - 参数滑块：水平滑块 + 数值输入框 + 单位标签
  - 范围设置：可调整滑块的最小/最大范围（范围设置按钮）
  - 步进设置：自动计算步进值
  - 操作按钮：应用、重置、自动仿真开关
  - 空状态提示：无参数时显示提示信息
- [X] **参数提取规则**（由 `domain/simulation/service/parameter_extractor.py` 实现）：
  - 从 `.param` 语句中提取参数名和当前值
  - 从电阻、电容、电感元件中提取数值
  - 从电压源、电流源中提取数值
  - 自动识别参数单位和合理范围（0.1x ~ 10x）
  - 支持单位前缀解析（f/p/n/u/m/k/meg/g/t）
- [X] **实时仿真模式**：
  - 开启"自动仿真"开关后，参数变化自动触发仿真
  - 使用防抖动机制，参数停止变化 500ms 后触发仿真
  - 通过 `simulation_requested` 信号和 `EVENT_TUNING_REQUEST_SIMULATION` 事件通知
- [ ] **参数变化记录**（后续阶段实现）：
  - 记录每次参数调整的历史
  - 支持撤销/重做参数调整
  - 显示参数变化对性能指标的影响
...
- [X] **信号定义**：
  - `parameters_changed(dict)` - 参数变更信号
  - `apply_requested(dict)` - 请求应用参数到电路文件
  - `reset_requested()` - 请求重置所有参数
  - `simulation_requested(dict)` - 请求触发仿真
- [X] **被调用方**：`simulation_tab.py`

##### 4.11.1.4.1 `domain/simulation/service/parameter_extractor.py` - 参数提取服务

> **设计原则**：纯领域逻辑，不依赖 UI，支持多种参数来源

- [X] **文件路径**：`domain/simulation/service/parameter_extractor.py`
- [X] **职责**：从电路文件中提取可调参数
- [X] **核心类**：
  - `ParameterExtractor` - 参数提取器
  - `TunableParameter(dataclass)` - 可调参数数据类
  - `ParameterExtractionResult(dataclass)` - 提取结果数据类
  - `ParameterType(Enum)` - 参数类型枚举
- [X] **支持的参数类型**：
  - `.param` 语句定义的参数
  - 电阻值（R 元件）
  - 电容值（C 元件）
  ...
  - 电流源值（I 元件）
- [X] **核心方法**：
  - `extract_from_file(file_path)` - 从文件提取参数
  - `extract_from_content(content, file_path)` - 从内容提取参数
  - `format_value_with_unit(value, unit)` - 格式化数值
- [X] **单位前缀支持**：f/p/n/u/m/k/meg/g/t
- [X] **模块级单例**：`parameter_extractor`

##### 4.11.1.5 `StatusIndicator` - 状态指示器组件（内联于 simulation_tab.py）

> **实现说明**：`StatusIndicator` 已内联实现于 `simulation_tab.py` 中，因为它是一个简单的 UI 组件（约80行），仅被 `SimulationTab` 使用，无需独立文件。

- [X] **文件路径**：`presentation/panels/simulation/simulation_tab.py`（内联实现）
- [X] **职责**：专注于工作流状态的可视化提示
- [X] **核心类**：`StatusIndicator(QFrame)`
- [X] **核心功能**：
  - `show_awaiting_confirmation()` - 显示等待确认状态
...
> **设计说明**：基于 pyqtgraph 的交互式波形图表组件，支持缩放、平移、双光标测量
>
> **技术选型变更**：原设计使用 Plotly + QWebEngineView，改为 pyqtgraph 方案
> - pyqtgraph 是纯 Python/Qt 实现，无需嵌入 Web 引擎
> - 原生支持大数据量（百万点）高性能渲染
> - 内置缩放、平移、光标功能，与 Qt 事件系统无缝集成
> - 内存占用更低，响应更快

- [x] **文件路径**：`presentation/panels/simulation/waveform_widget.py`
- [x] **职责**：渲染交互式波形图表，支持缩放、平移、测量
- [x] **技术选型**：pyqtgraph.PlotWidget（纯 Qt 实现）
- [x] **核心功能**：
  - `load_waveform(result, signal_name)` - 加载波形数据（调用 `WaveformDataService`）
  - `add_waveform(result, signal_name)` - 叠加显示波形
  - `clear_waveforms()` - 清空所有波形
  - `set_cursor_a(x_position)` - 设置光标 A 位置
  - `set_cursor_b(x_position)` - 设置光标 B 位置
  - `get_measurement()` - 获取双光标测量结果
  - `export_image(path)` - 导出图表图片
- [x] **数据获取**：调用 `domain/simulation/data/waveform_data_service.py` 获取降采样数据
- [x] **性能优化**：
  - 初始加载使用最低分辨率层（500点）
  - 缩放时动态请求更高分辨率数据（通过 sigRangeChanged 信号）
  - 防抖处理：100ms 内的连续缩放操作合并
- [x] **UI 特性**：
  - 双光标测量（可拖拽的垂直线）
  - 多信号叠加显示（不同颜色）
  - 图例显示/隐藏
  - 网格线显示
  - 自动缩放按钮
- [x] **被调用方**：`simulation_tab.py`、`chart_viewer.py`

##### 4.11.1.11 `measurement_tool.py` - 双光标测量工具

> **设计参考**：借鉴 LTspice 的交互式测量功能

- [x] **文件路径**：`presentation/panels/simulation/measurement_tool.py`
- [x] **职责**：提供双光标测量 UI，显示测量结果
- [x] **UI 结构**：
  - 光标 A 位置输入框 + 数值显示
  - 光标 B 位置输入框 + 数值显示
  - 差值显示区（ΔX、ΔY、斜率、频率）
...
- [x] **被调用方**：`chart_viewer.py`

##### 4.11.1.12 `raw_data_table.py` - 原始数据表格

> **设计说明**：使用虚拟滚动技术显示大量仿真数据

- [x] **文件路径**：`presentation/panels/simulation/raw_data_table.py`
- [x] **职责**：以表格形式显示仿真原始数据，支持虚拟滚动
- [x] **技术选型**：`QTableView` + 自定义 `QAbstractTableModel`
- [x] **核心功能**：
...
- [x] **文件路径**：`presentation/panels/simulation/output_log_viewer.py`
- [x] **职责**：显示 ngspice 原始输出日志，便于调试
- [x] **UI 结构**：
...
- [ ] **声明式指标提取验收**：
  - .MEASURE 语句注入：能正确将测量语句注入网表
  - .MEASURE 结果解析：能正确解析 ngspice 输出中的测量结果
  - 测量结果存储：测量结果正确存储在 SimulationResult.measurements 中
  - 与设计目标对比：测量结果能与设计目标对比，计算达标状态
- [ ] **高级图表生成验收**：
  - 蒙特卡洛直方图、良率曲线正确生成
  - PVT 角点对比图正确生成
  - 参数扫描曲线、等高线图正确生成
