...
> **⚠️ 与下栏仿真面板的职责分工（关键设计决策）**：
> - **本面板（右栏信息面板）**：专注于**轻量级信息聚合与快速查看**
>   - 仿真状态摘要卡片（成功/失败、耗时、关键指标数值）
>   - RAG 检索结果摘要
>   - 元器件搜索结果摘要
>   - 操作记录摘要
>   - 点击卡片可跳转到下栏详情
> - **下栏仿真面板（阶段四）**：专注于**仿真结果的详细展示与交互操作**
>   - 波形图表查看器（双光标测量、缩放、导出）
>   - 高级分析结果标签页（PVT/蒙特卡洛/参数扫描等）
>   - 电路图查看器
>   - 原始数据表格
> - **禁止重复实现**：本面板不实现图表查看、数据导出、波形测量等功能，这些由下栏面板负责

> **设计目标**：整合 RAG 索引、仿真摘要、元器件商城、工具执行等多类重要输出信息，提供集中的快速查看和导航功能

> **设计原则**：
> - 信息聚合：将分散在各模块的**摘要信息**统一展示
> - 轻量展示：只展示关键指标和状态，详情跳转到专门面板
> - 实时更新：订阅各模块事件，自动刷新显示
> - 快速导航：点击卡片跳转到对应的详情面板
...
│   ├── simulation_info_collector.py  # 仿真信息收集（仅摘要）
...
> **⚠️ 职责边界**：本收集器仅收集仿真**摘要信息**，详细的图表查看、数据导出、波形测量等功能由下栏仿真面板（阶段四）负责

- [ ] **文件路径**：`presentation/panels/info_panel/collectors/simulation_info_collector.py`
- [ ] **职责**：收集仿真状态摘要和关键指标，提供跳转到下栏详情的入口
- [ ] **订阅事件**：
  - `SIMULATION_COMPLETE` - 仿真完成
  - `SIMULATION_ERROR` - 仿真错误
- [ ] **收集内容（仅摘要）**：
  - 仿真状态摘要（分析类型、耗时、成功/失败）
  - 关键指标数值（增益、带宽、相位裕度等，仅数值）
  - 错误摘要（错误类型、简短描述）
- [ ] **不收集的内容（由下栏面板负责）**：
  - 图表路径列表（图表查看在下栏）
  - 详细错误上下文（错误详情在下栏）
  - 原始仿真数据（数据表格在下栏）
- [ ] **卡片类型**：
  - `simulation_summary` - 仿真状态摘要卡片（含跳转按钮）
  - `metrics_summary` - 关键指标摘要卡片（含跳转按钮）
  - `simulation_error_summary` - 错误摘要卡片（含跳转按钮）
- [ ] **跳转功能**：
  - 卡片包含"查看详情"按钮
  - 点击后发布 `EVENT_NAVIGATE_TO_SIMULATION_PANEL` 事件
  - 下栏面板订阅此事件并切换到对应标签页
...
> **⚠️ 职责分工**：本面板仅展示仿真摘要信息，详细图表和数据由下栏仿真面板负责
...
- [ ] **不收集的数据（由下栏面板负责）**：
  ...
  - 详细错误上下文和恢复建议
- [ ] **跳转事件**：
  - 卡片点击发布 `EVENT_NAVIGATE_TO_SIMULATION_PANEL` 事件
  - 下栏面板订阅此事件并切换到对应标签页
- [ ] **格式化职责**：由 `SimulationSummaryFormatter` 负责生成摘要展示文本
- [ ] **InfoCard 内容示例（仅摘要）**：
  ```python
  InfoCardContent(
      inline_data={
          ...
          "duration_ms": 1234,
          "success": True,
          "key_metrics": {"gain_db": 18.5, "bandwidth_hz": 1200000, "phase_margin_deg": 65.2}
      },
      references=[]  # 不存储图表引用，图表由下栏面板负责
  )
  ```
