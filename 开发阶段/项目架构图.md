...
flowchart TB
    ...
    RAW --> SE
    SE -->|_parse_exception| SER[SpiceErrorRecovery<br/>恢复策略]
    SE -->|成功| SR[SimulationResult]
...
> **面板职责边界说明**：
> - **下栏面板**：专注于仿真结果的**详细展示与交互操作**（图表查看、数据导出、波形测量）
> - **右栏信息面板**：专注于**轻量级信息聚合与快速导航**（摘要卡片、跳转链接）
> - 两者通过事件通信，点击右栏卡片可跳转到下栏详情
...
graph TB
    subgraph MainWindow["MainWindow 主窗口"]
        ...
        subgraph CentralWidget["中央区域"]
            ...
            subgraph RightPanel["右栏 30%<br/>轻量信息聚合"]
                TAB1[对话标签页]
                TAB2[信息标签页<br/>摘要卡片+跳转]
                TAB3[元器件标签页]
                TAB4[调试标签页]
            end
        end
        
        subgraph BottomPanel["下栏 20%<br/>详细展示与交互"]
            SIM_TAB[仿真结果标签页<br/>图表/数据/测量]
            RPT_TAB[报告生成标签页]
            SCH_TAB[电路图标签页<br/>交互式浏览]
        end
...
```mermaid
flowchart TB
    ERROR[仿真错误/异常] --> PARSE[SpiceExecutor<br/>_parse_exception]
    
    PARSE --> TYPE{错误类型分类}
    
    TYPE -->|语法错误| SYN[匹配 syntax/parse 关键词]
    TYPE -->|模型缺失| MOD[匹配 model/not found 关键词]
    TYPE -->|节点浮空| NODE[匹配 floating/no dc path 关键词]
    TYPE -->|收敛失败| CONV[匹配 convergence 关键词]
